...
#[test]
fn default_conversion_embeds_bundled_metric_fonts_for_common_word_families() {
    let input =
        br"{\rtf1\ansi{\fonttbl{\f0 Arial;}{\f1 Times New Roman;}}\f0 Default Arial AB\par\f1 Default Serif \u337?D\par}";
    let output = convert_rtf_to_pdf(
        input,
        &ConvertOptions {
            diagnostics: true,
    ...
    for expected in [
        ...
        assert!(
            output
                ...
                .windows(expected.len())
                .any(|window| window == expected),
            "expected default bundled passive font marker {:?}",
            String::from_utf8_lossy(expected)
        );
    }
        ...
        assert!(
            output.diagnostics.iter().all(|diagnostic| !diagnostic
                ...
                && !diagnostic
                    .message
                    .contains(&format!("font '{family}' substituted"))),
            "default bundled metric asset should suppress {family} base-font diagnostics: {:?}",
            output.diagnostics
        );
    }
    assert!(
    ...
    for forbidden in [
        ...
        assert!(
            !output
                ...
                .windows(forbidden.len())
                .any(|window| window == forbidden),
            "source font metadata or active PDF marker leaked: {:?}",
            String::from_utf8_lossy(forbidden)
        );
    }
}
