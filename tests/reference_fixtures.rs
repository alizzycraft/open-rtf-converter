use std::path::Path;

use lopdf::Document as PdfDocument;
use lopdf::content::Content;
use open_rtf_converter::pdf::audit_passive_pdf_bytes;
use open_rtf_converter::{ConvertOptions, convert_rtf_to_pdf};

const MANIFEST: &str = include_str!("../fixtures/reference/expected-policy.json");

#[test]
fn word_reference_policy_manifest_covers_existing_visual_fixtures() {
    assert!(MANIFEST.contains("\"schema\": 1"));
    assert!(
        MANIFEST.contains("development references only"),
        "manifest should document that Word is not a production dependency"
    );

    for fixture in [
        "fixtures/simple.rtf",
        "fixtures/table-ish.rtf",
        "fixtures/weird.rtf",
        "fixtures/object-result.rtf",
        "fixtures/png-alpha.rtf",
        "fixtures/png-trns.rtf",
        "fixtures/windows-bitmap.rtf",
        "fixtures/dib-grayscale-passive.rtf",
        "fixtures/wmf-vector-passive.rtf",
        "fixtures/wmf-text-passive.rtf",
        "fixtures/wmf-cliprect-passive.rtf",
        "fixtures/wmf-hatch-brush-passive.rtf",
        "fixtures/wmf-dashed-pen-passive.rtf",
        "fixtures/wmf-arc-family-passive.rtf",
        "fixtures/wmf-polybezier-passive.rtf",
        "fixtures/wmf-polybezierto-passive.rtf",
        "fixtures/wmf-polypolygon-passive.rtf",
        "fixtures/wmf-setpixel-passive.rtf",
        "fixtures/wmf-roundrect-passive.rtf",
        "fixtures/wmf-line-poly-paths-passive.rtf",
        "fixtures/wmf-patblt-passive.rtf",
        "fixtures/wmf-stretchblt-patcopy-passive.rtf",
        "fixtures/wmf-bitblt-patcopy-passive.rtf",
        "fixtures/wmf-dibbitblt-patcopy-passive.rtf",
        "fixtures/wmf-dibstretchblt-patcopy-passive.rtf",
        "fixtures/wmf-stretchdib-patcopy-passive.rtf",
        "fixtures/wmf-srccopy-stretchdib-passive.rtf",
        "fixtures/wmf-setdib-to-dev-passive.rtf",
        "fixtures/wmf-srccopy-dib-blits-passive.rtf",
        "fixtures/wmf-srccopy-cropped-dib-passive.rtf",
        "fixtures/wmf-srccopy-cropped-jpeg-dib-passive.rtf",
        "fixtures/emf-vector-passive.rtf",
        "fixtures/emf-text-passive.rtf",
        "fixtures/emf-cliprect-passive.rtf",
        "fixtures/emf-hatch-brush-passive.rtf",
        "fixtures/emf-arc-family-passive.rtf",
        "fixtures/emf-roundrect-passive.rtf",
        "fixtures/emf-setpixelv-passive.rtf",
        "fixtures/emf-polyline-polygon-passive.rtf",
        "fixtures/emf-polypolygon-passive.rtf",
        "fixtures/emf-bezier-passive.rtf",
        "fixtures/emf-polydraw-passive.rtf",
        "fixtures/emf-anglearc-passive.rtf",
        "fixtures/emf-savedc-restoredc-passive.rtf",
        "fixtures/static-shape-wrap.rtf",
        "fixtures/shape-wrap-distance-passive.rtf",
        "fixtures/floating-table-positioning.rtf",
        "fixtures/office-math-passive.rtf",
        "fixtures/office-math-structures-passive.rtf",
        "fixtures/office-math-flow-passive.rtf",
        "fixtures/office-math-limits-arrays-passive.rtf",
        "fixtures/office-math-nary-visible-passive.rtf",
        "fixtures/field-hyperlink-passive.rtf",
        "fixtures/generated-reference-fields-passive.rtf",
        "fixtures/mail-merge-prompt-fields-passive.rtf",
        "fixtures/template-barcode-embed-fields-passive.rtf",
        "fixtures/environment-layout-clock-fields-passive.rtf",
        "fixtures/nonvisible-storage-fields-passive.rtf",
        "fixtures/document-property-fields-passive.rtf",
        "fixtures/info-custom-docvariable-fields-passive.rtf",
        "fixtures/generated-counter-fields-passive.rtf",
        "fixtures/cross-reference-fields-passive.rtf",
        "fixtures/styleref-resultless-passive.rtf",
        "fixtures/styleref-multilevel-passive.rtf",
        "fixtures/literal-control-drawing-fields-passive.rtf",
        "fixtures/timestamp-stat-fields-passive.rtf",
        "fixtures/line-numbering-passive.rtf",
        "fixtures/page-border-art-passive.rtf",
        "fixtures/page-border-scope-passive.rtf",
        "fixtures/character-typography-passive.rtf",
        "fixtures/character-spacing-units-passive.rtf",
        "fixtures/character-scaling-spacing-passive.rtf",
        "fixtures/scaled-justification-passive.rtf",
        "fixtures/distributed-run-boundary-passive.rtf",
        "fixtures/section-grid-page-number-passive.rtf",
        "fixtures/page-number-format-restart-passive.rtf",
        "fixtures/table-spacing-autofit-passive.rtf",
        "fixtures/table-padding-passive.rtf",
        "fixtures/table-row-height-passive.rtf",
        "fixtures/table-cell-text-direction-passive.rtf",
        "fixtures/table-cell-vertical-align-passive.rtf",
        "fixtures/table-header-repeat-passive.rtf",
        "fixtures/tall-table-row-split-passive.rtf",
        "fixtures/table-row-keep-together-passive.rtf",
        "fixtures/table-row-keep-follow-passive.rtf",
        "fixtures/paragraph-keep-pagination-passive.rtf",
        "fixtures/widow-control-passive.rtf",
        "fixtures/no-wrap-passive.rtf",
        "fixtures/page-break-before-passive.rtf",
        "fixtures/section-break-kinds-passive.rtf",
        "fixtures/odd-section-break-passive.rtf",
        "fixtures/even-section-break-passive.rtf",
        "fixtures/page-vertical-alignment-passive.rtf",
        "fixtures/page-gutter-passive.rtf",
        "fixtures/landscape-zero-geometry-passive.rtf",
        "fixtures/alignment-direction-passive.rtf",
        "fixtures/paragraph-indent-aliases-passive.rtf",
        "fixtures/paragraph-spacing-passive.rtf",
        "fixtures/line-spacing-passive.rtf",
        "fixtures/soft-breaks-passive.rtf",
        "fixtures/drop-cap-passive.rtf",
        "fixtures/framed-drop-cap-passive.rtf",
        "fixtures/framed-drop-cap-derived-dimensions-passive.rtf",
        "fixtures/text-effects-passive.rtf",
        "fixtures/text-decoration-passive.rtf",
        "fixtures/character-highlight-border-passive.rtf",
        "fixtures/hyphenation-passive.rtf",
        "fixtures/form-field-shading-passive.rtf",
        "fixtures/note-placement-passive.rtf",
        "fixtures/note-restart-numbering-passive.rtf",
        "fixtures/shape-rotation-passive.rtf",
        "fixtures/section-columns-passive.rtf",
        "fixtures/column-section-break-passive.rtf",
        "fixtures/tab-alignment-passive.rtf",
        "fixtures/tab-leader-spacing-passive.rtf",
        "fixtures/tab-leader-variants-passive.rtf",
        "fixtures/legacy-textbox-passive.rtf",
        "fixtures/shading-patterns-passive.rtf",
        "fixtures/paragraph-borders-passive.rtf",
        "fixtures/table-borders-passive.rtf",
        "fixtures/carried-table-row-definitions-passive.rtf",
        "fixtures/intbl-zero-body-flow-passive.rtf",
        "fixtures/table-merged-cells-passive.rtf",
        "fixtures/nested-table-flatten-passive.rtf",
        "fixtures/old-style-list-passive.rtf",
        "fixtures/section-number-passive.rtf",
        "fixtures/picture-scaling-passive.rtf",
        "fixtures/picture-adjustments-passive.rtf",
        "fixtures/jpeg-trailing-payload-passive.rtf",
        "fixtures/associated-character-passive.rtf",
        "fixtures/font-family-hints-passive.rtf",
        "fixtures/font-charset-greek-passive.rtf",
        "fixtures/font-charset-cyrillic-passive.rtf",
        "fixtures/font-charset-central-european-passive.rtf",
        "fixtures/font-charset-turkish-passive.rtf",
        "fixtures/font-charset-baltic-passive.rtf",
        "fixtures/font-code-page-passive.rtf",
        "fixtures/font-charset-legacy-code-page-passive.rtf",
        "fixtures/zero-width-formatting-passive.rtf",
        "fixtures/symbol-font-passive.rtf",
        "fixtures/symbol-field-passive.rtf",
        "fixtures/pntext-symbol-list-passive.rtf",
        "fixtures/wingdings-checkbox-passive.rtf",
        "fixtures/wingdings2-checkbox-passive.rtf",
        "fixtures/wingdings3-arrows-passive.rtf",
        "fixtures/webdings-checkbox-passive.rtf",
        "fixtures/segoe-ui-symbol-checkbox-passive.rtf",
        "fixtures/form-checkbox-passive.rtf",
        "fixtures/form-text-dropdown-passive.rtf",
        "fixtures/active-control-fields-passive.rtf",
        "fixtures/document-protection-passive.rtf",
        "fixtures/mailmerge-object-metadata-passive.rtf",
        "fixtures/private-metadata-passive.rtf",
        "fixtures/review-bookmark-metadata-passive.rtf",
        "fixtures/hidden-animation-passive.rtf",
        "fixtures/font-template-metadata-passive.rtf",
        "fixtures/macro-script-metadata-passive.rtf",
        "fixtures/html-xml-metadata-passive.rtf",
        "fixtures/word-metadata-session-passive.rtf",
        "fixtures/stylesheet-metadata-passive.rtf",
        "fixtures/structural-metadata-passive.rtf",
        "fixtures/word-grid-outline-metadata-passive.rtf",
        "fixtures/stylesheet-semantics-passive.rtf",
        "fixtures/word-layout-compatibility-passive.rtf",
        "fixtures/section-pages-passive.rtf",
        "fixtures/header-footer-variants-passive.rtf",
        "fixtures/header-footer-distance-passive.rtf",
        "fixtures/background-shape-passive.rtf",
        "fixtures/shape-z-order-text-passive.rtf",
        "fixtures/shape-arrowheads-passive.rtf",
        "fixtures/shape-line-dashing-passive.rtf",
        "fixtures/shape-line-cap-passive.rtf",
        "fixtures/shape-line-join-passive.rtf",
        "fixtures/shape-pattern-fill-passive.rtf",
        "fixtures/shape-gradient-fill-passive.rtf",
        "fixtures/shape-shadow-opacity-passive.rtf",
        "fixtures/shape-fill-fore-color-passive.rtf",
        "fixtures/shape-line-fore-color-passive.rtf",
        "fixtures/shape-fill-back-color-passive.rtf",
        "fixtures/shape-line-back-color-passive.rtf",
        "fixtures/shape-fill-flag-passive.rtf",
        "fixtures/shape-line-flag-passive.rtf",
        "fixtures/shape-disabled-fill-passive.rtf",
        "fixtures/shape-disabled-line-passive.rtf",
        "fixtures/shape-hidden-passive.rtf",
        "fixtures/shape-flip-passive.rtf",
        "docs/sample.rtf",
    ] {
        assert!(
            Path::new(fixture).is_file(),
            "manifest references missing fixture {fixture}"
        );
        assert!(
            MANIFEST.contains(&format!("\"input\": \"{fixture}\"")),
            "manifest must classify {fixture}"
        );
    }

    for category in [
        "must_match_closely",
        "acceptable_approximation",
        "intentional_security_difference",
    ] {
        assert!(
            MANIFEST.contains(&format!("\"category\": \"{category}\"")),
            "manifest must include category {category}"
        );
    }

    assert!(
        MANIFEST.contains("\"word_reference_status\": \"pending_word_export\""),
        "current fixtures should explicitly mark missing Word references instead of implying coverage"
    );
    assert!(
        MANIFEST.contains("\"word_reference_pdf\": null"),
        "missing Word reference PDFs should be explicit"
    );
    assert!(
        MANIFEST.contains("\"intentional_security_differences\""),
        "security-sensitive fixtures must document intentional Word differences"
    );
    assert!(
        MANIFEST.contains("\"known_gaps\""),
        "visual fixtures must track missing comparison evidence"
    );
    assert_eq!(
        MANIFEST.matches("\"expected_diagnostics\"").count(),
        reference_fixtures().len(),
        "each manifest fixture should explicitly document expected diagnostics"
    );
    assert_eq!(
        MANIFEST.matches("\"forbidden_pdf_markers\"").count(),
        reference_fixtures().len(),
        "each manifest fixture should explicitly document source/control/PDF markers that must not reach PDF bytes"
    );
    assert_eq!(
        MANIFEST.matches("\"expected_pdf_markers\"").count(),
        reference_fixtures().len(),
        "each manifest fixture should explicitly document passive PDF markers required by the executable fixture gate"
    );
}

#[test]
fn available_word_reference_pdfs_match_manifest_contract() {
    let mut failures = Vec::new();
    for fixture in manifest_reference_fixtures() {
        match fixture.word_reference_status.as_str() {
            "pending_word_export" => assert!(
                fixture.word_reference_pdf.is_none(),
                "{} cannot mark a Word reference pending while naming a PDF",
                fixture.input
            ),
            "available" => {
                let Some(path) = fixture.word_reference_pdf.as_deref() else {
                    failures.push(format!("{} marks a missing Word reference", fixture.input));
                    continue;
                };
                if !Path::new(path).is_file() {
                    failures.push(format!(
                        "{} names missing Word reference PDF {}",
                        fixture.input, path
                    ));
                    continue;
                }
                let Ok(pdf) = PdfDocument::load(path) else {
                    failures.push(format!("{} Word reference PDF is invalid", fixture.input));
                    continue;
                };
                let page_count = pdf.get_pages().len();
                if page_count != fixture.word_expected_pages {
                    failures.push(format!(
                        "{} Word reference page count drifted: expected {}, got {}",
                        fixture.input, fixture.word_expected_pages, page_count
                    ));
                }
                let rendered_text = normalize_pdf_contract_text(&decoded_pdf_text(&pdf));
                for expected in &fixture.word_must_preserve_text {
                    let expected = normalize_pdf_contract_text(expected);
                    if !pdf_contract_text_contains(&rendered_text, &expected) {
                        failures.push(format!(
                            "{} Word reference PDF did not contain {:?}",
                            fixture.input, expected
                        ));
                    }
                }
            }
            status => panic!(
                "{} has unsupported word_reference_status {:?}",
                fixture.input, status
            ),
        }
    }
    assert!(
        failures.is_empty(),
        "Word reference contract failures:\n{}",
        failures.join("\n")
    );
}

#[test]
fn reference_policy_fixtures_match_current_passive_converter_output() {
    for fixture in manifest_reference_fixtures() {
        let input = std::fs::read(&fixture.input).unwrap_or_else(|error| {
            panic!(
                "failed to read reference fixture {}: {error}",
                fixture.input
            )
        });
        let output = convert_rtf_to_pdf(
            &input,
            &ConvertOptions {
                diagnostics: true,
                ..ConvertOptions::default()
            },
        )
        .unwrap_or_else(|error| panic!("failed to convert {}: {error}", fixture.input));

        assert_eq!(
            output.pages, fixture.expected_pages,
            "{} should render the expected page count",
            fixture.input
        );
        audit_passive_pdf_bytes(&output.pdf).unwrap_or_else(|error| {
            panic!("{} emitted active PDF content: {error}", fixture.input)
        });

        let pdf = PdfDocument::load_mem(&output.pdf)
            .unwrap_or_else(|error| panic!("{} emitted invalid PDF: {error}", fixture.input));
        assert_eq!(
            pdf.get_pages().len(),
            fixture.expected_pages,
            "{} PDF page tree should match report",
            fixture.input
        );
        let rendered_text = decoded_pdf_text(&pdf);
        for expected in &fixture.must_preserve_text {
            assert!(
                rendered_text.contains(expected),
                "{} rendered PDF text did not contain {:?}; text was {:?}",
                fixture.input,
                expected,
                rendered_text
            );
        }
        for forbidden in &fixture.must_not_leak {
            let forbidden = forbidden.as_bytes();
            assert!(
                !output
                    .pdf
                    .windows(forbidden.len())
                    .any(|window| window == forbidden),
                "{} leaked forbidden source/control bytes {:?}",
                fixture.input,
                String::from_utf8_lossy(forbidden)
            );
        }
        for expected in &fixture.must_contain_pdf {
            let expected = expected.as_bytes();
            assert!(
                output
                    .pdf
                    .windows(expected.len())
                    .any(|window| window == expected),
                "{} rendered PDF did not contain expected passive marker {:?}",
                fixture.input,
                String::from_utf8_lossy(expected)
            );
        }
        for expected in &fixture.must_emit_diagnostics {
            assert!(
                output
                    .diagnostics
                    .iter()
                    .any(|diagnostic| diagnostic.message.contains(expected)),
                "{} did not emit expected diagnostic {:?}; diagnostics were {:?}",
                fixture.input,
                expected,
                output.diagnostics
            );
        }
    }
}

#[test]
fn executable_reference_fixtures_follow_manifest_policy_entries() {
    let manifest_fixtures = manifest_reference_fixtures();
    assert_eq!(
        manifest_fixtures.len(),
        reference_fixtures().len(),
        "executable fixture count should stay in lockstep with manifest policy entries"
    );

    for fixture in reference_fixtures() {
        let manifest = manifest_fixtures
            .iter()
            .find(|manifest| manifest.input == fixture.input)
            .unwrap_or_else(|| panic!("manifest missing executable fixture {}", fixture.input));
        assert_eq!(
            manifest.expected_pages, fixture.expected_pages,
            "{} expected_pages drifted between manifest and executable gate",
            fixture.input
        );
        assert_eq!(
            manifest.must_preserve_text,
            fixture
                .must_preserve_text
                .iter()
                .map(|value| (*value).to_string())
                .collect::<Vec<_>>(),
            "{} must_preserve_text drifted between manifest and executable gate",
            fixture.input
        );
        assert_eq!(
            manifest.must_not_leak,
            fixture
                .must_not_leak
                .iter()
                .map(|marker| String::from_utf8_lossy(marker).into_owned())
                .collect::<Vec<_>>(),
            "{} forbidden_pdf_markers drifted between manifest and executable gate",
            fixture.input
        );
        assert_eq!(
            manifest.must_contain_pdf,
            fixture
                .must_contain_pdf
                .iter()
                .map(|marker| String::from_utf8_lossy(marker).into_owned())
                .collect::<Vec<_>>(),
            "{} expected_pdf_markers drifted between manifest and executable gate",
            fixture.input
        );
        assert_eq!(
            manifest.must_emit_diagnostics,
            fixture
                .must_emit_diagnostics
                .iter()
                .map(|value| (*value).to_string())
                .collect::<Vec<_>>(),
            "{} expected_diagnostics drifted between manifest and executable gate",
            fixture.input
        );
    }
}

#[derive(Debug)]
struct ManifestReferenceFixture {
    input: String,
    word_reference_status: String,
    word_reference_pdf: Option<String>,
    expected_pages: usize,
    word_expected_pages: usize,
    must_preserve_text: Vec<String>,
    word_must_preserve_text: Vec<String>,
    must_not_leak: Vec<String>,
    must_contain_pdf: Vec<String>,
    must_emit_diagnostics: Vec<String>,
}

struct ReferenceFixture {
    input: &'static str,
    expected_pages: usize,
    must_preserve_text: &'static [&'static str],
    must_not_leak: &'static [&'static [u8]],
    must_contain_pdf: &'static [&'static [u8]],
    must_emit_diagnostics: &'static [&'static str],
}

fn manifest_reference_fixtures() -> Vec<ManifestReferenceFixture> {
    let fixtures = json_array_for_key(MANIFEST, "fixtures");
    split_json_objects(fixtures)
        .into_iter()
        .map(|object| {
            let must_preserve_text = json_string_array_for_key(object, "must_preserve_text");
            let word_must_preserve_text =
                json_optional_string_array_for_key(object, "word_must_preserve_text")
                    .unwrap_or_default();
            let expected_pages = json_usize_for_key(object, "expected_pages");
            ManifestReferenceFixture {
                input: json_string_for_key(object, "input"),
                word_reference_status: json_string_for_key(object, "word_reference_status"),
                word_reference_pdf: json_nullable_string_for_key(object, "word_reference_pdf"),
                expected_pages,
                word_expected_pages: json_optional_usize_for_key(object, "word_expected_pages")
                    .unwrap_or(expected_pages),
                must_preserve_text,
                word_must_preserve_text,
                must_not_leak: json_string_array_for_key(object, "forbidden_pdf_markers"),
                must_contain_pdf: json_string_array_for_key(object, "expected_pdf_markers"),
                must_emit_diagnostics: json_string_array_for_key(object, "expected_diagnostics"),
            }
        })
        .collect()
}

fn reference_fixtures() -> &'static [ReferenceFixture] {
    &[
        ReferenceFixture {
            input: "fixtures/simple.rtf",
            expected_pages: 2,
            must_preserve_text: &[
                "Hello from open-rtf-converter",
                "Centered paragraph with",
                "Second page text",
            ],
            must_not_leak: &[b"fonttbl", b"colortbl", b"/JavaScript", b"/EmbeddedFile"],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/table-ish.rtf",
            expected_pages: 1,
            must_preserve_text: &["Name", "Value", "Alpha", "Beta", "After table text"],
            must_not_leak: &[b"trowd", b"cellx", b"/JavaScript", b"/EmbeddedFile"],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/weird.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "visible text should survive",
                "Escaped braces: {sample}",
                "hex: ABC",
            ],
            must_not_leak: &[
                b"unknownDestination",
                b"madeup123",
                b"/JavaScript",
                b"/EmbeddedFile",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/object-result.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before object result.",
                "visible fallback",
                "After object result.",
            ],
            must_not_leak: &[
                b"objemb",
                b"objdata",
                b"414243",
                b"JavaScript",
                b"EmbeddedFile",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/png-alpha.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before alpha image.", "After alpha image."],
            must_not_leak: &[
                b"pngblip",
                b"IHDR",
                b"IDAT",
                b"IEND",
                b"/JavaScript",
                b"/EmbeddedFile",
            ],
            must_contain_pdf: &[b"/SMask"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/png-trns.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before indexed transparency image.",
                "After indexed transparency image.",
            ],
            must_not_leak: &[
                b"pngblip",
                b"IHDR",
                b"PLTE",
                b"tRNS",
                b"IDAT",
                b"IEND",
                b"/JavaScript",
                b"/EmbeddedFile",
            ],
            must_contain_pdf: &[b"/SMask"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/windows-bitmap.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before Windows bitmap image.",
                "After Windows bitmap image.",
            ],
            must_not_leak: &[
                b"wbitmap",
                b"wbmplanes",
                b"wbmbitspixel",
                b"wbmwidthbytes",
                b"ff0000ffffff0000ff00ff00",
                b"/JavaScript",
                b"/EmbeddedFile",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "legacy Windows bitmap picture suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/dib-grayscale-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before DIB grayscale.", "After DIB grayscale."],
            must_not_leak: &[
                b"dibitmap",
                b"picprop",
                b"pictureGray",
                b"pFragments",
                b"hidden-fragment",
                b"280000000200000001000000010018000000000008000000000000000000000000000000000000000000ff00ff000000",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "parameterless DIB picture suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/wmf-vector-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF vector.", "After WMF vector."],
            must_not_leak: &[
                b"wmetafile",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"dcdcdc",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" re", b" S"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-text-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF text.", "Hi", "After WMF text."],
            must_not_leak: &[
                b"wmetafile",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"0a32",
                b"4869",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-cliprect-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF clip.", "After WMF clip."],
            must_not_leak: &[
                b"wmetafile",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"1604",
                b"dcdcdc",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" n", b" re", b" S"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-hatch-brush-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF hatch.", "After WMF hatch."],
            must_not_leak: &[
                b"wmetafile",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"02fc",
                b"0200ff0000000000",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" n", b" m", b" l", b" S"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-dashed-pen-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF dashed pen.", "After WMF dashed pen."],
            must_not_leak: &[
                b"wmetafile",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"02fa",
                b"010004000000",
                b"ff000000",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" d", b" m", b" l", b" S"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-arc-family-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF arcs.", "After WMF arcs."],
            must_not_leak: &[
                b"wmetafile",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"1708",
                b"3008",
                b"1a08",
                b"dcdcdc",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" m", b" l", b" f"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-polybezier-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF polybezier.", "After WMF polybezier."],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"3c2814",
                b"051004000a000a00",
                b"4600320050001400",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-polybezierto-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF polybezierto.", "After WMF polybezierto."],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"3c2814",
                b"041003001e000500",
                b"4600320050001400",
                b"13023c005a00",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" l", b" S"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-polypolygon-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF polypolygon.", "After WMF polypolygon."],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"0538",
                b"06010200",
                b"dcdcdc",
                b"8c000a008c004600",
                b"6400190064003700",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" m", b" l", b" f"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-setpixel-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF setpixel.", "After WMF setpixel."],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"041f",
                b"ff0000",
                b"20004000",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" m", b" l", b" S"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-roundrect-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF roundrect.", "After WMF roundrect."],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"061c",
                b"00ff00",
                b"1c06140028005000",
                b"b4000a001400",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" c", b" f"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-line-poly-paths-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF poly paths.", "After WMF poly paths."],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"0324",
                b"0325",
                b"dcdcdc",
                b"2403030014002800",
                b"2503040014004600",
                b"8c004600b4005a00",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" m", b" l", b" S"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-patblt-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF patblt.", "After WMF patblt."],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"061d",
                b"00f00021",
                b"ff0000",
                b"1d062100f0001400",
                b"28001e003200",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" re", b" f"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-stretchblt-patcopy-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before WMF stretchblt patcopy.",
                "After WMF stretchblt patcopy.",
            ],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"0b23",
                b"00f00021",
                b"285078",
                b"RAW-SRCCOPY-PAYLOAD",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "odd trailing WMF hex nibble ignored for Word-compatible passive recovery",
            ],
        },
        ReferenceFixture {
            input: "fixtures/wmf-bitblt-patcopy-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before WMF bitblt patcopy.",
                "After WMF bitblt patcopy.",
            ],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"0922",
                b"00f00021",
                b"1e6eb4",
                b"RAW-SRCCOPY-BITBLT",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-dibbitblt-patcopy-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before WMF dibbitblt patcopy.",
                "After WMF dibbitblt patcopy.",
            ],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"0940",
                b"00f00021",
                b"785028",
                b"RAW-DIBBITBLT-PAYLOAD",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" re", b" f"],
            must_emit_diagnostics: &[
                "WMF picture rendered as bounded passive vector preview with 1 unsupported record(s) skipped",
            ],
        },
        ReferenceFixture {
            input: "fixtures/wmf-dibstretchblt-patcopy-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before WMF dibstretchblt patcopy.",
                "After WMF dibstretchblt patcopy.",
            ],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"0b41",
                b"00f00021",
                b"2846a0",
                b"RAW-DIBSTRETCH-PAYLOAD",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-stretchdib-patcopy-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before WMF stretchdib patcopy.",
                "After WMF stretchdib patcopy.",
            ],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"0f43",
                b"00f00021",
                b"a04628",
                b"RAW-STRETCHDIB-PAYLOAD",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-srccopy-stretchdib-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before WMF srccopy stretchdib.",
                "After WMF srccopy stretchdib.",
            ],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"0f43",
                b"00cc0020",
                b"280000000200000001000000010018",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b"/Subtype /Image"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-setdib-to-dev-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before WMF setdib to dev.", "After WMF setdib to dev."],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"0d33",
                b"280000000200000001000000010018",
                b"TRAILING-WMF-SETDIBTODEV",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-srccopy-dib-blits-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before WMF srccopy dib blits.",
                "After WMF srccopy dib blits.",
            ],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"0940",
                b"0b41",
                b"00cc0020",
                b"280000000200000001000000010018",
                b"TRAILING-WMF-SRCCOPY-DIB",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b"/Subtype /Image"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-srccopy-cropped-dib-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before WMF srccopy cropped dib.",
                "After WMF srccopy cropped dib.",
            ],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"0f43",
                b"0940",
                b"0b41",
                b"00cc0020",
                b"280000000300000002000000010018",
                b"TRAILING-WMF-CROPPED-DIB",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b"/Subtype /Image"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wmf-srccopy-cropped-jpeg-dib-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before WMF srccopy cropped jpeg dib.",
                "After WMF srccopy cropped jpeg dib.",
            ],
            must_not_leak: &[
                b"wmetafile8",
                b"picw200",
                b"pich100",
                b"picwgoal",
                b"pichgoal",
                b"010009",
                b"0f43",
                b"0b41",
                b"00cc0020",
                b"TRAILING-WMF-STRETCHDIB-JPEG-CROP",
                b"TRAILING-WMF-STRETCHDIB-JPEG-CROP-DIB",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b"/Subtype /Image"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/emf-vector-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF vector.", "After EMF vector."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"e7030000",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/emf-text-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF text.", "After EMF text."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"48006900",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/emf-cliprect-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF clip.", "After EMF clip."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"1e000000",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/emf-hatch-brush-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF hatch.", "After EMF hatch."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"27000000",
                b"145ab400",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/emf-arc-family-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF arcs.", "After EMF arcs."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"2d000000",
                b"2e000000",
                b"2f000000",
                b"37000000",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/emf-roundrect-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF roundrect.", "After EMF roundrect."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"2c000000",
                b"5a00000050000000",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/emf-setpixelv-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF setpixelv.", "After EMF setpixelv."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"0f00000014000000",
                b"14283c00",
                b"dcb47800",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/emf-polyline-polygon-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF poly paths.", "After EMF poly paths."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"0400000034000000",
                b"0300000034000000",
                b"c800000064000000",
                b"3c00000046000000",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/emf-polypolygon-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF polypolygon.", "After EMF polypolygon."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"130000000c000000",
                b"2700000018000000",
                b"0800000068000000",
                b"8c00000046000000",
                b"6400000037000000",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/emf-bezier-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF bezier.", "After EMF bezier."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"020000003c000000",
                b"0500000034000000",
                b"550000002c000000",
                b"5800000028000000",
                b"6400280078004600c8006400",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/emf-polydraw-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF polydraw.", "After EMF polydraw."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"380000005c000000",
                b"06020306040405",
                b"3600000010000000",
                b"5c0000002c000000",
                b"140032003c0032003c004600060202",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/emf-anglearc-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF anglearc.", "After EMF anglearc."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"1b00000010000000",
                b"290000001c000000",
                b"3c0000002800000028000000",
                b"0000b442",
                b"3600000010000000",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/emf-savedc-restoredc-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before EMF saved state.", "After EMF saved state."],
            must_not_leak: &[
                b"emfblip",
                b"picw160",
                b"pich80",
                b"picwgoal",
                b"pichgoal",
                b"20454d46",
                b"260000001c000000",
                b"c8141400",
                b"1428c800",
                b"2100000008000000",
                b"220000000c000000ffffffff",
                b"36000000100000002800000014000000",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "EMF picture with missing reference-device metrics suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/static-shape-wrap.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before wrapped static shape.",
                "Wrapped static shape text should flow beside the passive frame",
                "After wrapped static shape.",
            ],
            must_not_leak: &[
                b"shpinst",
                b"shpleft",
                b"shptop",
                b"shpright",
                b"shpbottom",
                b"shpwr",
                b"shpwrk",
                b"shapeType",
                b"pFragments",
                b"hidden-static-wrap-reference-payload",
                b"/JavaScript",
                b"/EmbeddedFile",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-wrap-distance-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before shape wrap distance.",
                "Wrapped paragraph should honor the passive shape wrap distances",
                "After shape wrap distance.",
            ],
            must_not_leak: &[
                b"shpinst",
                b"shpleft",
                b"shptop",
                b"shpright",
                b"shpbottom",
                b"shpwr",
                b"shpwrk",
                b"shapeType",
                b"dxWrapDistLeft",
                b"dxWrapDistRight",
                b"dyWrapDistTop",
                b"dyWrapDistBottom",
                b"pFragments",
                b"hostile-wrap-distance-reference-payload",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/floating-table-positioning.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before floating table.",
                "Floating left",
                "Floating right",
                "Follow left",
                "Follow right",
                "After floating table.",
            ],
            must_not_leak: &[
                b"tabsnoovrlp",
                b"trgaph",
                b"trleft",
                b"tphmrg",
                b"posx",
                b"pvmrg",
                b"posy",
                b"tdfrmtxt",
                b"cellx",
                b"trowd",
                b"/JavaScript",
                b"/EmbeddedFile",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "floating table no-overlap \\tabsnoovrlp captured as bounded passive row exclusion",
                "floating table horizontal position interpreted as bounded passive row offset",
                "floating table vertical position interpreted as bounded passive row offset",
                "floating table wrap distance interpreted as bounded passive row margin",
            ],
        },
        ReferenceFixture {
            input: "fixtures/office-math-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before math.", "E=mc2", "x+1", "y", "after math."],
            must_not_leak: &[
                b"mmath",
                b"moMath",
                b"mtext",
                b"msup",
                b"mnum",
                b"mden",
                b"xmlopen",
                b"hidden-office-math-reference-payload",
                b"objdata",
                b"414243",
                b"/JavaScript",
                b"/EmbeddedFile",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "Office math rendered as bounded passive math text",
                "active content removed: OLE object before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/office-math-structures-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before math structures.",
                "root-x",
                "over-z",
                "p+q",
                "after math structures.",
            ],
            must_not_leak: &[
                b"mmath",
                b"moMath",
                b"mrad",
                b"mradPr",
                b"mdeg",
                b"mdegHide",
                b"HIDDEN-DEGREE",
                b"mbar",
                b"mbarPr",
                b"mpos",
                b"mdPr",
                b"mbegChr",
                b"mendChr",
                b"mtext",
                b"calc.exe",
                b"launch.exe",
                b"objdata",
                b"414243",
                b"444546",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
            ],
            must_contain_pdf: &[b" S"],
            must_emit_diagnostics: &["Office math rendered as bounded passive math text"],
        },
        ReferenceFixture {
            input: "fixtures/office-math-flow-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before math flow.",
                "FirstSecond",
                "x+1",
                "sin x",
                "after math flow.",
            ],
            must_not_leak: &[
                b"mmath",
                b"moMath",
                b"mbox",
                b"mboxPr",
                b"mbrk",
                b"mgroupChr",
                b"mgroupChrPr",
                b"mchr",
                b"mfunc",
                b"mfuncPr",
                b"mfName",
                b"mctrlPr",
                b"mphant",
                b"mphantPr",
                b"mtext",
                b"calc.exe",
                b"example.invalid",
                b"objdata",
                b"414243",
                b"444546",
                b"454647",
                b"464748",
                b"HIDDEN-PHANTOM-PAYLOAD",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
            ],
            must_contain_pdf: &[b" S"],
            must_emit_diagnostics: &["Office math rendered as bounded passive math text"],
        },
        ReferenceFixture {
            input: "fixtures/office-math-limits-arrays-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before math arrays.",
                "X=1",
                "Y=2",
                "limx=0maxn",
                "i",
                "VisiblePhantom",
                "after math arrays.",
            ],
            must_not_leak: &[
                b"mmath",
                b"moMath",
                b"meqArr",
                b"meqArrPr",
                b"mlimLow",
                b"mlimLowPr",
                b"mlimUpp",
                b"mlimUppPr",
                b"mlim",
                b"mnary",
                b"mnaryPr",
                b"mchr",
                b"msubHide",
                b"msupHide",
                b"msub",
                b"msup",
                b"HIDDEN-SUB-LIMIT",
                b"HIDDEN-SUP-LIMIT",
                b"mphant",
                b"mphantPr",
                b"mshow",
                b"HIDDEN-PHANTOM-PAYLOAD",
                b"mtext",
                b"calc.exe",
                b"launch.exe",
                b"example.com/payload",
                b"objdata",
                b"414243",
                b"444546",
                b"454647",
                b"464748",
                b"474849",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &["Office math rendered as bounded passive math text"],
        },
        ReferenceFixture {
            input: "fixtures/office-math-nary-visible-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before inline ",
                "i=1na",
                " between under-over ",
                "k=1mb",
                " after visible n-ary math.",
            ],
            must_not_leak: &[
                b"mmath",
                b"moMath",
                b"mnary",
                b"mnaryPr",
                b"mchr",
                b"mlimLoc",
                b"msubHide",
                b"msupHide",
                b"msub",
                b"msup",
                b"mtext",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &["Office math rendered as bounded passive math text"],
        },
        ReferenceFixture {
            input: "fixtures/field-hyperlink-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before field.",
                "Visible safe link text",
                "after link.",
                "[Field removed: no passive result]",
                "after external field.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"fldrslt",
                b"HYPERLINK",
                b"INCLUDEPICTURE",
                b"example.com",
                b"active?token=414243",
                b"pixel.png",
                b"/URI",
                b"/Annots",
                b"/Action",
                b"/JavaScript",
                b"/EmbeddedFile",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering stored result for field HYPERLINK without executing field instruction",
                "external field INCLUDEPICTURE rendered as passive placeholder without fetching external resource",
            ],
        },
        ReferenceFixture {
            input: "fixtures/generated-reference-fields-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before generated references.",
                "TOC [Field removed: no passive result]",
                "Index [Field removed: no passive result]",
                "Citation [Field removed: no passive result]",
                "Bibliography [Field removed: no passive result]",
                "Authorities [Field removed: no passive result]",
                "Visible after generated references.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"TOC",
                b"INDEX",
                b"CITATION",
                b"BIBLIOGRAPHY",
                b"TOA",
                b"HiddenSource",
                b"Hidden separator",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "generated field TOC rendered as passive placeholder without synthesizing generated field output",
                "generated field INDEX rendered as passive placeholder without synthesizing generated field output",
                "generated field CITATION rendered as passive placeholder without synthesizing generated field output",
                "generated field BIBLIOGRAPHY rendered as passive placeholder without synthesizing generated field output",
                "generated field TOA rendered as passive placeholder without synthesizing generated field output",
            ],
        },
        ReferenceFixture {
            input: "fixtures/mail-merge-prompt-fields-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before mail merge and prompts.",
                "Ask",
                "Ask ref [Field removed: no passive result]",
                "Fill Secret default",
                "Unsafe fill [Field removed: no passive result]",
                "Next",
                "Next if",
                "Skip if",
                "Record [Field removed: no passive result]",
                "Sequence [Field removed: no passive result]",
                "Merge name",
                "Client",
                "Address [Field removed: no passive result]",
                "Greeting Dear Sir or Madam,",
                "Unsafe greeting [Field removed: no passive result]",
                "Visible after mail merge and prompts.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"ASK",
                b"FILLIN",
                b"NEXTIF",
                b"SKIPIF",
                b"MERGEREC",
                b"MERGESEQ",
                b"MERGEFIELD",
                b"ADDRESSBLOCK",
                b"GREETINGLINE",
                b"Hidden prompt",
                b"Hidden default",
                b"Secret prompt",
                b"Unsafe prompt",
                b"City",
                b"Paris",
                b"Status",
                b"Hidden",
                b"_COMPANY_",
                b"_ADDRESS1_",
                b"_TITLE0_",
                b"_LAST0_",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "prompt field FILLIN rendered from passive default without requesting user input",
                "prompt field FILLIN rendered as passive placeholder without requesting user input",
                "mail-merge field MERGEREC rendered as passive placeholder without executing merge data source",
                "mail-merge field MERGESEQ rendered as passive placeholder without executing merge data source",
                "mail-merge field ADDRESSBLOCK rendered as passive placeholder without executing merge data source",
                "mail-merge field GREETINGLINE rendered from passive fallback without executing merge data source",
                "mail-merge field GREETINGLINE rendered as passive placeholder without executing merge data source",
            ],
        },
        ReferenceFixture {
            input: "fixtures/template-barcode-embed-fields-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before template and embedded fields.",
                "Auto text [Field removed: no passive result]",
                "Auto text list [Field removed: no passive result]",
                "Barcode [Field removed: no passive result]",
                "Display barcode [Field removed: no passive result]",
                "Merge barcode [Field removed: no passive result]",
                "Math (1,2)",
                "Embed [Field removed: no passive result]",
                "Go button \"Visible jump\"",
                "Visible after template and embedded fields.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"AUTOTEXT",
                b"AUTOTEXTLIST",
                b"BARCODE",
                b"DISPLAYBARCODE",
                b"MERGEBARCODE",
                b"EMBED",
                b"GOTOBUTTON",
                b"HiddenBlock",
                b"Hidden menu",
                b"HiddenStyle",
                b"Hidden address",
                b"Hidden code",
                b"Hidden merge value",
                b"HiddenBookmark",
                b"Word.Document.8",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "building-block field AUTOTEXT rendered as passive placeholder without reading template storage",
                "building-block field AUTOTEXTLIST rendered as passive placeholder without reading template storage",
                "generated field BARCODE rendered as passive placeholder without synthesizing generated field output",
                "generated field DISPLAYBARCODE rendered as passive placeholder without synthesizing generated field output",
                "generated field MERGEBARCODE rendered as passive placeholder without synthesizing generated field output",
                "rendering passive field EQ without executing field instruction",
                "embedded object field EMBED rendered as passive placeholder without activating embedded object",
                "rendering passive field GOTOBUTTON without executing field instruction",
            ],
        },
        ReferenceFixture {
            input: "fixtures/environment-layout-clock-fields-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before environment fields.",
                "Size ",
                "Path [Field removed: no passive result]",
                "Template [Field removed: no passive result]",
                "User [Field removed: no passive result]",
                "Initials [Field removed: no passive result]",
                "Address [Field removed: no passive result]",
                "Advance 240 120after advance.",
                "Date ",
                "Time ",
                "Visible after environment fields.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"FILESIZE",
                b"FILENAME",
                b"TEMPLATE",
                b"USERNAME",
                b"USERINITIALS",
                b"USERADDRESS",
                b"ADVANCE",
                b"DATE",
                b"TIME",
                b"nofbytes",
                b"host-sentinel",
                b"Users\\",
                b"open-rtf-converter",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "resultless FILESIZE suppressed for Word-compatible passive rendering",
                "environment field FILENAME rendered as passive placeholder without exposing converter host state",
                "environment field TEMPLATE rendered as passive placeholder without exposing converter host state",
                "environment field USERNAME rendered as passive placeholder without exposing converter host state",
                "environment field USERINITIALS rendered as passive placeholder without exposing converter host state",
                "environment field USERADDRESS rendered as passive placeholder without exposing converter host state",
                "malformed ADVANCE arguments recovered as Word-compatible passive literal text",
                "resultless field DATE omitted for Word-compatible passive rendering without dynamic evaluation",
                "resultless field TIME omitted for Word-compatible passive rendering without dynamic evaluation",
            ],
        },
        ReferenceFixture {
            input: "fixtures/nonvisible-storage-fields-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before non-visible storage fields.",
                "Addin  after addin.",
                "Private  after private.",
                "Print  after print.",
                "RD  after rd.",
                "Index  after index.",
                "TOC entry  after toc.",
                "Authority entry  after authority.",
                "Visible after non-visible storage fields.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"ADDIN",
                b"PRIVATE",
                b"PRINT",
                b"Hidden addin payload",
                b"414243",
                b"hidden private",
                b"hidden printer command",
                b"secret",
                b"source.rtf",
                b"https://example.com",
                b"Hidden TOC",
                b"Hidden authority",
                b"[Field removed",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active non-visible field ADDIN stripped",
                "active non-visible field PRIVATE stripped",
                "active non-visible field PRINT stripped",
                "external non-visible field RD stripped",
                "non-visible field XE stripped",
                "non-visible field TC stripped",
                "non-visible field TA stripped",
            ],
        },
        ReferenceFixture {
            input: "fixtures/document-property-fields-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before document properties.",
                "Doc Safe title",
                "tail / ALICE",
                "Shortcuts Safe title",
                "tail / ALICE / Bob / Comment text",
                "Extended Alice Manager / CONTOSO",
                "TAIL / Internal Use",
                "Numeric revision XLII edit 0017 paragraphs 3 lines 012 bytes 4,096",
                "Visible after document properties.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"DOCPROPERTY",
                b"LASTSAVEDBY",
                b"COMMENTS",
                b"MANAGER",
                b"COMPANY",
                b"CATEGORY",
                b"HYPERLINK",
                b"https://example.com",
                b"Hidden link",
                b"Hidden company",
                b"Revision Number",
                b"Total Editing Time",
                b"Number of Paragraphs",
                b"Number of Lines",
                b"Number of Bytes",
                b"version",
                b"edmins",
                b"nofparas",
                b"noflines",
                b"nofbytes",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering passive field DOCPROPERTY without executing field instruction",
                "rendering passive field TITLE without executing field instruction",
                "rendering passive field AUTHOR without executing field instruction",
                "rendering passive field LASTSAVEDBY without executing field instruction",
                "rendering passive field COMMENTS without executing field instruction",
                "rendering passive field MANAGER without executing field instruction",
                "rendering passive field COMPANY without executing field instruction",
                "rendering passive field CATEGORY without executing field instruction",
            ],
        },
        ReferenceFixture {
            input: "fixtures/info-custom-docvariable-fields-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before info metadata.",
                "Info Info title",
                "tail / ALICE / Bob / BOB / Comment text / https://example.test/base / file [Field removed: no passive result]",
                "Info stats created 2024-07-05 saved 2025-01-02 00:00:00 revision IX edit 0017 paragraphs 3 lines 012 bytes 4,096",
                "Custom CONTOSO",
                "TAIL",
                "Variable FABRIKAM",
                "TAIL missing [Field removed: no passive result]",
                "Visible after info metadata.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"INFO",
                b"DOCVARIABLE",
                b"Client Name",
                b"Client Var",
                b"Last Author",
                b"Last Modified By",
                b"Filename",
                b"Creation Date",
                b"Last Save Time",
                b"Revision Number",
                b"Total Editing Time",
                b"Number of Paragraphs",
                b"Number of Lines",
                b"Number of Bytes",
                b"HYPERLINK",
                b"https://example.com",
                b"Hidden info link",
                b"Hidden custom link",
                b"Hidden docvar link",
                b"linked-source.docm",
                b"secret",
                b"linkval",
                b"docvar",
                b"staticval",
                b"creatim",
                b"revtim",
                b"version",
                b"edmins",
                b"nofparas",
                b"noflines",
                b"nofbytes",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: field instruction in metadata before safe model normalization",
                "rendering passive field INFO without executing field instruction",
                "rendering passive field DOCPROPERTY without executing field instruction",
                "rendering passive field DOCVARIABLE without executing field instruction",
                "field DOCVARIABLE has no stored result and was not evaluated dynamically",
            ],
        },
        ReferenceFixture {
            input: "fixtures/generated-counter-fields-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before generated counters.",
                "Figure 1 and Figure 2 Table 1",
                "Reset Figure 4 then 5 repeat 5 hidden  next 7",
                "Auto Clause 1.. Legal 11. Outline 1..",
                "List Item 1). Sub a). Named 2.. Named next 3.. Reset 7.. Next 8..",
                "Visible after generated counters.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"SEQ",
                b"AUTONUM",
                b"AUTONUMLGL",
                b"AUTONUMOUT",
                b"LISTNUM",
                b"LegalDefault",
                b"[Field removed",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering passive field SEQ without executing field instruction",
                "rendering passive field AUTONUM without executing field instruction",
                "rendering passive field AUTONUMLGL without executing field instruction",
                "rendering passive field AUTONUMOUT without executing field instruction",
                "rendering passive field LISTNUM without executing field instruction",
            ],
        },
        ReferenceFixture {
            input: "fixtures/cross-reference-fields-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before cross references.",
                "Before  copy CONTOSO VALUE missing [Field removed: no passive result]",
                "Bookmark Marked text visible Marked text unsafe [Field removed: no passive result]",
                "Target Target text page 1 missing page ?",
                "Note 1 note I missing note",
                "1. Footnote text",
                "Visible heading",
                "Style ref VISIBLE HEADING again Visible heading missing [Field removed: no passive result]",
                "Visible after cross references.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"SET Client",
                b"SET Unsafe",
                b"REF Client",
                b"REF Unsafe",
                b"PAGEREF",
                b"NOTEREF",
                b"STYLEREF",
                b"Heading One",
                b"VisibleMark",
                b"TargetBookmark",
                b"NoteRef",
                b"MissingTarget",
                b"MissingNote",
                b"PAYLOAD",
                b"bkmkstart",
                b"bkmkend",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering passive field SET without executing field instruction",
                "rendering passive field REF without executing field instruction",
                "field REF has no stored result and was not evaluated dynamically",
                "rendering passive field PAGEREF without executing field instruction",
                "rendering passive field NOTEREF without executing field instruction",
                "field NOTEREF has no stored result and was not evaluated dynamically",
                "rendering passive field STYLEREF without executing field instruction",
                "field STYLEREF has no stored result and was not evaluated dynamically",
            ],
        },
        ReferenceFixture {
            input: "fixtures/styleref-resultless-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible title",
                "Numbered title",
        "Ref text NUMBERED TITLE",
        "Ref number 0",
        "Ref contexts full 0 relative 0 malformed [Field removed: no passive result]",
            ],
            must_not_leak: &[
                b"STYLEREF",
                b"TitleStyle",
                b"nfoo",
                b"HYPERLINK",
                b"https://example.com",
                b"Hidden link",
                b"fldinst",
                b"fldrslt",
                b"stylesheet",
                b"listtable",
                b"listoverridetable",
                b"leveltext",
                b"levelnumbers",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering passive field STYLEREF without executing field instruction",
                "field STYLEREF has no stored result and was not evaluated dynamically",
            ],
        },
        ReferenceFixture {
            input: "fixtures/styleref-multilevel-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Root target",
                "Child target",
                "Leaf target",
                "Number 1.1.1 relative 1.1.1 full 1.1.1",
                "Context number 1.1.1 relative 1.1.1",
                "full 1.1.1",
            ],
            must_not_leak: &[
                b"STYLEREF",
                b"TargetStyle",
                b"ContextStyle",
                b"fldinst",
                b"stylesheet",
                b"listtable",
                b"listoverridetable",
                b"leveltext",
                b"levelnumbers",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering passive field STYLEREF without executing field instruction",
            ],
        },
        ReferenceFixture {
            input: "fixtures/literal-control-drawing-fields-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before literal and drawing fields.",
                "Quote Visible quoted text unicode",
                "formatted MIXED CASE",
                "If safe Greater false Different formatted Checked Status",
                "Compare safe 1 false 0 roman I",
                "Macro Visible button text unsafe literal [Field removed: no passive result] unsafe macro [Field removed: no passive result]",
                "Stored shape Visible drawing fallback resultless shape [Field removed: no passive result]",
                "Visible after literal and drawing fields.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"fldrslt",
                b"QUOTE",
                b"COMPARE",
                b"MACROBUTTON",
                b"SHAPE",
                b"MERGEFORMAT",
                b"LaunchPayload",
                b"mixed Case",
                b"Upper",
                b"Caps",
                b"ROMAN",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering passive field QUOTE without executing field instruction",
                "rendering passive field IF without executing field instruction",
                "rendering passive field COMPARE without executing field instruction",
                "rendering passive field MACROBUTTON without executing field instruction",
                "rendering stored result for field SHAPE without executing field instruction",
                "drawing field SHAPE rendered as passive placeholder without synthesizing drawing canvas output",
            ],
        },
        ReferenceFixture {
            input: "fixtures/timestamp-stat-fields-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before timestamp stats.",
                "Created  saved  printed  bad saved",
                "Edit  edit raw  missing edit",
                "Revision  revision raw  missing revision",
                "Dynamic date  time",
                "Visible after timestamp stats.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"CREATEDATE",
                b"SAVEDATE",
                b"PRINTDATE",
                b"EDITTIME",
                b"REVNUM",
                b"DATE",
                b"TIME",
                b"creatim",
                b"revtim",
                b"printim",
                b"edmins",
                b"version",
                b"unknown-token",
                b"host-sentinel",
                b"HYPERLINK",
                b"https://example.com",
                b"Hidden title",
                b"Hidden link",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: field instruction in metadata before safe model normalization",
                "resultless field CREATEDATE omitted for Word-compatible passive rendering without dynamic evaluation",
                "resultless field SAVEDATE omitted for Word-compatible passive rendering without dynamic evaluation",
                "resultless field PRINTDATE omitted for Word-compatible passive rendering without dynamic evaluation",
                "resultless field EDITTIME omitted for Word-compatible passive rendering without dynamic evaluation",
                "resultless field REVNUM omitted for Word-compatible passive rendering without dynamic evaluation",
                "resultless field DATE omitted for Word-compatible passive rendering without dynamic evaluation",
                "resultless field TIME omitted for Word-compatible passive rendering without dynamic evaluation",
            ],
        },
        ReferenceFixture {
            input: "fixtures/line-numbering-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "1Numbered first line.",
                "2Numbered second line.",
                "3Numbered third line.",
            ],
            must_not_leak: &[
                b"linemod",
                b"linex",
                b"lineppage",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &["line numbering rendered as bounded passive margin text"],
        },
        ReferenceFixture {
            input: "fixtures/page-border-art-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Body inside passive page border fallback."],
            must_not_leak: &[
                b"pgbrdrt",
                b"brdrart",
                b"brdrw80",
                b"brsp240",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "page border art 42 rendered as bounded passive vector template",
            ],
        },
        ReferenceFixture {
            input: "fixtures/page-border-scope-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Body inside scoped passive page border."],
            must_not_leak: &[
                b"pgbrdrhead",
                b"pgbrdrfoot",
                b"pgbrdropt",
                b"pgbrdrt",
                b"pgbrdrb",
                b"pgbrdrl",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" S"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/character-typography-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before typography.",
                "Visible styled typography",
                "After typography.",
            ],
            must_not_leak: &[
                b"kerning2",
                b"charscalex125",
                b"fittext720",
                b"objdata",
                b"414243",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "character kerning rendered as bounded passive pair spacing",
                "character fit-text rendered as bounded passive horizontal scaling",
                "active content removed: OLE object before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/character-spacing-units-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Plain: ABCD",
                "Expnd +4: ABCD",
                "Expnd -4: ABCD",
                "Twips +20: ABCD",
                "Twips -20: ABCD",
                "Expnd then twips: ABCD",
                "Twips then expnd: ABCD",
                "Reset: ABCDEFGH",
            ],
            must_not_leak: &[
                b"expnd4",
                b"expnd-4",
                b"expndtw20",
                b"expndtw-20",
                b"expndtw10",
                b"expnd0",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/character-scaling-spacing-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Plain: ABCD",
                "Scale 50: ABCD",
                "Scale 150: ABCD",
                "Scale 200: ABCD",
                "Scale zero: ABCD",
                "Scale default: ABCD",
                "Scale reset: ABCDEFGH",
                "Scale 50 spacing: ABCD",
                "Scale 150 spacing: ABCD",
                "Spacing then scale: ABCD",
                "Scale precedence: ABCD",
            ],
            must_not_leak: &[
                b"charscalex50",
                b"charscalex100",
                b"charscalex150",
                b"charscalex200",
                b"charscalex0",
                b"expndtw20",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/scaled-justification-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Plain one two three four five six",
                "Scale50 one two three four five six seven eight nine ten eleven",
                "Scale150 one two",
                "fourteen fifteen",
                "sixteen.",
            ],
            must_not_leak: &[
                b"charscalex50",
                b"charscalex150",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/section-grid-page-number-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Page 1", "Visible section grid body.", "Second grid line."],
            must_not_leak: &[
                b"pgnx",
                b"pgny",
                b"sectlinegrid",
                b"sectdefaultcl",
                b"sectexpand",
                b"sectspecifycl",
                b"sectspecifyl",
                b"chpgn",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "page number position rendered as bounded passive header/footer coordinates",
                "section line grid applied as bounded passive paragraph line pitch",
                "section default text grid cleared bounded passive paragraph line pitch",
                "section text grid interpreted through bounded passive paragraph layout",
            ],
        },
        ReferenceFixture {
            input: "fixtures/page-number-format-restart-passive.rtf",
            expected_pages: 4,
            must_preserve_text: &[
                "Page I",
                "Page II",
                "Page 1",
                "Page 2",
                "First roman page.",
                "Second roman page.",
                "Restarted decimal section.",
                "Continued decimal page.",
            ],
            must_not_leak: &[
                b"pgnucrm",
                b"pgndec",
                b"pgnstarts",
                b"pgnrestart",
                b"chpgn",
                b"sectd",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/table-spacing-autofit-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before table spacing.",
                "Unit left",
                "Unit right",
                "Authored left",
                "Authored right",
                "After table spacing.",
            ],
            must_not_leak: &[
                b"trowd",
                b"trautofit",
                b"nogrowautofit",
                b"trspdl",
                b"trspdfl",
                b"trspdr",
                b"trspdfr",
                b"trspdt",
                b"trspdft",
                b"trspdb",
                b"trspdfb",
                b"clspdl",
                b"clspdfl",
                b"clspdr",
                b"clspdfr",
                b"clftsWidth",
                b"clwWidth",
                b"trftsWidth",
                b"trwWidth",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "table row autofit interpreted through bounded passive table width layout",
                "table autofit growth disabled through bounded passive authored widths",
                "table cell spacing rendered as bounded passive border gaps",
                "table row without authored cell boundaries omitted to match Word recovery",
            ],
        },
        ReferenceFixture {
            input: "fixtures/table-padding-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before table padding.",
                "Row padded",
                "Cell padded",
                "After table padding.",
            ],
            must_not_leak: &[
                b"trowd",
                b"trgaph",
                b"trpaddfl",
                b"trpaddfr",
                b"trpaddft",
                b"trpaddfb",
                b"trpaddl",
                b"trpaddr",
                b"trpaddt",
                b"trpaddb",
                b"clpadfl",
                b"clpadfr",
                b"clpadft",
                b"clpadfb",
                b"clpadl",
                b"clpadr",
                b"clpadt",
                b"clpadb",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "table padding and spacing units interpreted through bounded twip layout",
            ],
        },
        ReferenceFixture {
            input: "fixtures/table-row-height-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before table row height.",
                "Minimum row",
                "Normal neighbor",
                "Exact row first line",
                "Exact neighbor",
                "After table row height.",
            ],
            must_not_leak: &[
                b"colortbl",
                b"red240",
                b"trowd",
                b"trgaph",
                b"trrh",
                b"clcbpat",
                b"clbrdrt",
                b"clbrdrl",
                b"clbrdrb",
                b"clbrdrr",
                b"brdrw",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/table-cell-text-direction-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before text direction table.",
                "Vertical ABC",
                "Alias vertical",
                "Bottom top XY",
                "Flat text",
                "After text direction table.",
            ],
            must_not_leak: &[
                b"trowd",
                b"trgaph",
                b"trrh",
                b"cltxtbrl",
                b"cltxtbrlv",
                b"cltxbtlr",
                b"cltxlrtb",
                b"clbrdrt",
                b"clbrdrl",
                b"clbrdrb",
                b"clbrdrr",
                b"brdrw",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/table-cell-vertical-align-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before vertical align table.",
                "Top aligned",
                "Center aligned",
                "Bottom aligned",
                "After vertical align table.",
            ],
            must_not_leak: &[
                b"trowd",
                b"trgaph",
                b"trrh",
                b"clvertalt",
                b"clvertalc",
                b"clvertalb",
                b"clbrdrt",
                b"clbrdrl",
                b"clbrdrb",
                b"clbrdrr",
                b"brdrw",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/table-header-repeat-passive.rtf",
            expected_pages: 5,
            must_preserve_text: &[
                "Before repeating table header.",
                "Header row",
                "Body row 01",
                "Body row 18",
                "Body row 36",
                "After repeating table header.",
            ],
            must_not_leak: &[
                b"trowd",
                b"trhdr",
                b"trhdr0",
                b"trrh",
                b"clcbpat",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/tall-table-row-split-passive.rtf",
            expected_pages: 4,
            must_preserve_text: &[
                "Before split table.",
                "Safe fallback",
                "Split header",
                "Split row line 000",
                "Split row line 069",
                "After split table.",
            ],
            must_not_leak: &[
                b"objemb",
                b"objdata",
                b"414243",
                b"JavaScript",
                b"EmbeddedFile",
                b"trowd",
                b"trhdr",
                b"trrh",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: OLE object before safe model normalization",
                "rendering safe embedded object result and stripping active object data",
            ],
        },
        ReferenceFixture {
            input: "fixtures/table-row-keep-together-passive.rtf",
            expected_pages: 2,
            must_preserve_text: &[
                "Before keep-together table.",
                "Filler row 1",
                "Filler row 3",
                "Kept tall row first line",
                "Kept tall row second line",
                "Normal row",
                "After keep-together table.",
            ],
            must_not_leak: &[
                b"trowd",
                b"trrh",
                b"trkeep",
                b"trkeep0",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/table-row-keep-follow-passive.rtf",
            expected_pages: 2,
            must_preserve_text: &[
                "Before keep-follow table.",
                "Filler row 1",
                "Filler row 3",
                "Keep follow row",
                "Follower row",
                "After keep-follow table.",
            ],
            must_not_leak: &[
                b"trowd",
                b"trrh",
                b"trkeepfollow",
                b"trkeepfollow0",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/paragraph-keep-pagination-passive.rtf",
            expected_pages: 3,
            must_preserve_text: &[
                "Filler one",
                "Filler three",
                "Kept line one",
                "Kept line two",
                "Kept line three",
                "Keep next heading",
                "Follower paragraph",
                "Plain tail",
            ],
            must_not_leak: &[
                b"keep",
                b"keepn",
                b"keep0",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/widow-control-passive.rtf",
            expected_pages: 4,
            must_preserve_text: &[
                "Document default",
                "Carries pagination control",
                "Filler one",
                "Filler six",
                "Alpha",
                "Beta",
                "Gamma",
                "Plain tail",
                "Restored tail",
                "Alias plain tail",
            ],
            must_not_leak: &[
                b"widowctrl",
                b"widctlpar",
                b"nowidctlpar",
                b"nowidowctrl",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/no-wrap-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Alias Alpha Beta",
                "Alias wrapped paragraph",
                "Direct Alpha Beta",
                "Direct wrapped paragraph",
                "Cell Alpha Beta",
                "Cell wrapped content",
            ],
            must_not_leak: &[
                b"nowrap",
                b"nowwrap",
                b"clNoWrap",
                b"trowd",
                b"trrh",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/page-break-before-passive.rtf",
            expected_pages: 2,
            must_preserve_text: &["First page text", "Second page text", "Still second page"],
            must_not_leak: &[
                b"pagebb",
                b"pagebb0",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/section-break-kinds-passive.rtf",
            expected_pages: 3,
            must_preserve_text: &["Alpha flow.", "Beta flow.", "Gamma flow."],
            must_not_leak: &[
                b"sbknone",
                b"sbkpage",
                b"\\sect",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/odd-section-break-passive.rtf",
            expected_pages: 3,
            must_preserve_text: &["First parity section.", "Odd-start section."],
            must_not_leak: &[
                b"sbkodd",
                b"\\sect",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/even-section-break-passive.rtf",
            expected_pages: 3,
            must_preserve_text: &["First parity section.", "Even-start section."],
            must_not_leak: &[
                b"sbkeven",
                b"\\sect",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/page-vertical-alignment-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Center page body", "Top reset body"],
            must_not_leak: &[
                b"vertalc",
                b"vertalt",
                b"vertalb",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/page-gutter-passive.rtf",
            expected_pages: 3,
            must_preserve_text: &[
                "Bound text",
                "Odd page body",
                "Even page body",
                "Right binding text",
            ],
            must_not_leak: &[
                b"gutter",
                b"guttersxn",
                b"gutterprl",
                b"rtlgutter",
                b"margmirsxn",
                b"margmirror",
                b"facingp",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/landscape-zero-geometry-passive.rtf",
            expected_pages: 3,
            must_preserve_text: &["Portrait body", "portrait page two", "Later section"],
            must_not_leak: &[
                b"landscape",
                b"lndscpsxn",
                b"paperw",
                b"paperh",
                b"pgwsxn",
                b"pghsxn",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/alignment-direction-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "distributed one",
                "thai distributed",
                "thai justified",
                "Right placed",
                "Left placed",
                "Normal",
                "LTR text",
            ],
            must_not_leak: &[
                b"\\qd",
                b"\\qk",
                b"\\qt",
                b"rtlpar",
                b"ltrpar",
                b"rtlch",
                b"ltrch",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "right-to-left character direction rendered by bounded passive visual text order",
            ],
        },
        ReferenceFixture {
            input: "fixtures/distributed-run-boundary-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Plain distributed final line.",
                "Mixed bold plain italic final line.",
                "Scale50 bold plain italic final line.",
                "Scale150 bold plain",
                "italic final.",
            ],
            must_not_leak: &[
                b"\\qd",
                b"charscalex50",
                b"charscalex150",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/paragraph-indent-aliases-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Alias indented paragraph", "Normal following paragraph"],
            must_not_leak: &[
                b"lin720",
                b"rin360",
                b"fin-240",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/paragraph-spacing-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Auto spaced paragraph",
                "Manual spaced paragraph",
                "Context first paragraph",
                "Context second paragraph",
                "Normal tail paragraph",
            ],
            must_not_leak: &[
                b"sbauto",
                b"saauto",
                b"contextualspace",
                b"sb240",
                b"sa360",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
                b"/URI",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/line-spacing-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before line spacing.",
                "Double spaced line one",
                "Double spaced line two",
                "Exact spaced line one",
                "Exact spaced line two",
                "Clamped spacing line.",
                "After line spacing.",
            ],
            must_not_leak: &[
                b"sl480",
                b"slmult1",
                b"sl-360",
                b"slmult0",
                b"sl99999",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &["using automatic line spacing"],
        },
        ReferenceFixture {
            input: "fixtures/soft-breaks-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before soft line soft column soft page",
                "After soft breaks.",
            ],
            must_not_leak: &[
                b"softline",
                b"softcol",
                b"softpage",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/drop-cap-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Dropped paragraph continues",
                "enlarged first letter.",
                "Plain paragraph follows normal baseline flow.",
            ],
            must_not_leak: &[
                b"dropcapli",
                b"dropcapt",
                b"pard",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/framed-drop-cap-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "D",
                "First framed drop-cap paragraph",
                "body text around the retained passive",
                "I",
                "Second framed drop-cap paragraph",
                "body flow.",
                "Plain paragraph after both framed drop-cap",
            ],
            must_not_leak: &[
                b"dropcapli",
                b"dropcapt",
                b"absw",
                b"absh",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "paragraph frame dimensions interpreted through bounded passive flow layout",
            ],
        },
        ReferenceFixture {
            input: "fixtures/framed-drop-cap-derived-dimensions-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "W",
                "Width-only frame paragraph",
                "returning to full body width.",
                "H",
                "Height-only frame paragraph",
                "and ordinary flow.",
                "Plain paragraph after derived frame",
            ],
            must_not_leak: &[
                b"dropcapli",
                b"dropcapt",
                b"absw",
                b"absh",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "paragraph frame dimensions interpreted through bounded passive flow layout",
            ],
        },
        ReferenceFixture {
            input: "fixtures/framed-drop-cap-wrap-distance-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "No gap alpha beta gamma delta",
                "All gap alpha beta gamma delta",
                "X gap alpha beta gamma delta",
                "Y gap alpha beta gamma delta",
                "sigma tau.",
            ],
            must_not_leak: &[
                b"dropcapli",
                b"dropcapt",
                b"absw",
                b"absh",
                b"dxfrtext",
                b"dfrmtxtx",
                b"dfrmtxty",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "paragraph frame dimensions interpreted through bounded passive flow layout",
                "paragraph wrap distance interpreted through bounded passive flow layout",
            ],
        },
        ReferenceFixture {
            input: "fixtures/framed-drop-cap-positioning-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Normal alpha beta gamma",
                "Margin offset alpha beta",
                "Page offset alpha beta",
                "Centered alpha beta gamma",
                "Vertical alpha beta gamma",
                "Right alpha beta gamma",
            ],
            must_not_leak: &[
                b"dropcapli",
                b"dropcapt",
                b"absw",
                b"absh",
                b"phmrg",
                b"phpg",
                b"posx",
                b"posxc",
                b"posxr",
                b"pvpara",
                b"posy",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "paragraph frame dimensions interpreted through bounded passive flow layout",
                "paragraph frame horizontal position interpreted as bounded passive placement",
                "paragraph frame horizontal anchor \\phmrg interpreted as bounded passive origin",
                "paragraph frame horizontal alignment \\posxc interpreted as bounded passive placement",
                "paragraph frame vertical position interpreted as bounded passive placement",
            ],
        },
        ReferenceFixture {
            input: "fixtures/framed-drop-cap-vertical-alignment-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Zero alpha beta gamma",
                "Tops alpha beta gamma",
                "Cent alpha beta gamma",
                "Bott alpha beta gamma",
                "nu xi omicron.",
            ],
            must_not_leak: &[
                b"dropcapli",
                b"dropcapt",
                b"absw",
                b"absh",
                b"pvpara",
                b"posyt",
                b"posyc",
                b"posyb",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "paragraph frame dimensions interpreted through bounded passive flow layout",
            ],
        },
        ReferenceFixture {
            input: "fixtures/framed-drop-cap-absolute-overlap-passive.rtf",
            expected_pages: 2,
            must_preserve_text: &[
                "Margin prior alpha beta gamma",
                "Margin after alpha beta gamma",
                "Page first prior.",
                "Page prior alpha beta gamma",
                "Page after alpha beta gamma",
            ],
            must_not_leak: &[
                b"dropcapli",
                b"dropcapt",
                b"absw",
                b"absh",
                b"phmrg",
                b"phpg",
                b"pvmrg",
                b"pvpg",
                b"posx",
                b"posy",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "paragraph frame dimensions interpreted through bounded passive flow layout",
                "paragraph frame horizontal position interpreted as bounded passive placement",
                "paragraph frame horizontal anchor \\phmrg interpreted as bounded passive origin",
                "paragraph frame vertical anchor \\pvmrg interpreted as bounded passive origin",
                "paragraph frame vertical position interpreted as bounded passive placement",
            ],
        },
        ReferenceFixture {
            input: "fixtures/framed-drop-cap-natural-pagination-passive.rtf",
            expected_pages: 2,
            must_preserve_text: &[
                "Before A01 A02 A03",
                "A73 A74 A75",
                "A76 A77 A78 A79 A80.",
                "After natural pagination.",
            ],
            must_not_leak: &[
                b"dropcapli",
                b"dropcapt",
                b"absw",
                b"absh",
                b"phpg",
                b"pvpg",
                b"posx",
                b"posy",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "paragraph frame dimensions interpreted through bounded passive flow layout",
                "paragraph frame horizontal position interpreted as bounded passive placement",
                "paragraph frame horizontal anchor \\phpg interpreted as bounded passive origin",
                "paragraph frame vertical anchor \\pvpg interpreted as bounded passive origin",
                "paragraph frame vertical position interpreted as bounded passive placement",
            ],
        },
        ReferenceFixture {
            input: "fixtures/framed-drop-cap-facing-alignment-passive.rtf",
            expected_pages: 2,
            must_preserve_text: &[
                "Odd inside alpha beta gamma",
                "Odd outside alpha beta gamma",
                "Even inside alpha beta gamma",
                "Even outside alpha beta gamma",
            ],
            must_not_leak: &[
                b"dropcapli",
                b"dropcapt",
                b"absw",
                b"absh",
                b"facingp",
                b"phmrg",
                b"pvpg",
                b"posxi",
                b"posxo",
                b"posy",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "paragraph frame dimensions interpreted through bounded passive flow layout",
                "paragraph frame horizontal anchor \\phmrg interpreted as bounded passive origin",
                "paragraph frame horizontal alignment \\posxi interpreted as bounded passive placement",
                "paragraph frame horizontal alignment \\posxo interpreted as bounded passive placement",
                "paragraph frame vertical anchor \\pvpg interpreted as bounded passive origin",
                "paragraph frame vertical position interpreted as bounded passive placement",
            ],
        },
        ReferenceFixture {
            input: "fixtures/framed-drop-cap-negative-positioning-passive.rtf",
            expected_pages: 4,
            must_preserve_text: &[
                "Signed positive x alpha beta gamma",
                "Last signed x alpha beta",
                "Last positive x alpha beta",
                "Signed positive y alpha beta",
                "Page signed positive alpha beta",
                "Signed negative x alpha beta",
                "Signed negative y alpha beta",
            ],
            must_not_leak: &[
                b"dropcapli",
                b"dropcapt",
                b"absw",
                b"absh",
                b"phmrg",
                b"phpg",
                b"pvmrg",
                b"pvpg",
                b"posx",
                b"posy",
                b"posnegx",
                b"posnegy",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "paragraph frame dimensions interpreted through bounded passive flow layout",
                "paragraph frame horizontal anchor \\phmrg interpreted as bounded passive origin",
                "paragraph frame horizontal position interpreted as bounded passive placement",
                "paragraph frame vertical anchor \\pvmrg interpreted as bounded passive origin",
                "paragraph frame vertical position interpreted as bounded passive placement",
            ],
        },
        ReferenceFixture {
            input: "fixtures/text-effects-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "plain", "rimmed", "normal", "cast", "clear", "raised", "carved", "level", "LOUD",
                "quiet", "MIX",
            ],
            must_not_leak: &[
                b"outl0",
                b"shad0",
                b"embo ",
                b"impr0",
                b"caps0",
                b"scaps0",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/text-decoration-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Two words",
                "plain",
                "colored",
                "auto",
                "plain tail",
                "topmark",
                "clear",
                "second",
                "done",
            ],
            must_not_leak: &[
                b"ulw",
                b"ulc2",
                b"ulc0",
                b"ulnone",
                b"ol0",
                b"olnone",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" RG", b" S"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/character-highlight-border-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before character drawing.",
                "Marked text",
                "plain text",
                "Tinted text",
                "Bordered text",
                "After character drawing.",
            ],
            must_not_leak: &[
                b"highlight",
                b"chshdng",
                b"chcbpat",
                b"chbrdr",
                b"brdrs",
                b"brdrw9999",
                b"brdrcf",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" rg", b" re"],
            must_emit_diagnostics: &["border width clamped"],
        },
        ReferenceFixture {
            input: "fixtures/hyphenation-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Antidisestablis",
                "hmentarianis",
                "Characteristic",
                "ally",
            ],
            must_not_leak: &[
                b"hyphauto",
                b"hyphpar",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "document hyphenation rendered as bounded passive soft hyphenation",
                "paragraph hyphenation rendered as bounded passive soft hyphenation",
                "top margin 720720 twips exceeds",
                "bottom margin 720720 twips exceeds",
            ],
        },
        ReferenceFixture {
            input: "fixtures/form-field-shading-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before form field.", "Visible value", "After form field."],
            must_not_leak: &[
                b"formshade",
                b"FORMTEXT",
                b"formfield",
                b"HiddenName",
                b"HiddenDefault",
                b"launch.exe",
                b"datafield",
                b"414243",
                b"/AcroForm",
                b"/Widget",
                b"/AA",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "form-field shading is editor-only and omitted from passive print output",
                "rendering stored result for passive form field FORMTEXT without creating PDF form actions",
            ],
        },
        ReferenceFixture {
            input: "fixtures/note-placement-passive.rtf",
            expected_pages: 2,
            must_preserve_text: &[
                "Body with footnote",
                "Footnote text",
                "and endnote",
                "Endnote text",
                "Next section body.",
            ],
            must_not_leak: &[
                b"ftnbj",
                b"endnhere",
                b"chftn",
                b"footnote",
                b"endnote",
                b"objdata",
                b"414243",
                b"444546",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "footnotes placed at passive page bottom without active note behavior",
                "endnotes placed at passive section boundary without active note behavior",
                "active content removed: OLE object before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/note-restart-numbering-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Body1 Middle2 Endi",
                "1First note text",
                "2Second note text",
                "iFinal note text",
            ],
            must_not_leak: &[
                b"ftnrestart",
                b"ftnrstpg",
                b"ftnrstcont",
                b"aftnrestart",
                b"aftnrstpg",
                b"aftnrstcont",
                b"ftnstart",
                b"ftnnruc",
                b"aftnstart",
                b"aftnnalc",
                b"chftn",
                b"footnote",
                b"endnote",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/shape-rotation-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before tilted shape.", "After tilted shape."],
            must_not_leak: &[
                b"shpinst",
                b"shpleft",
                b"shptop",
                b"shpright",
                b"shpbottom",
                b"shapeType",
                b"rotation",
                b"pFragments",
                b"hidden-rotated-shape-reference-payload",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
                "shape rotation rendered as bounded passive static geometry",
            ],
        },
        ReferenceFixture {
            input: "fixtures/section-columns-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Left rail text.", "Right rail text."],
            must_not_leak: &[
                b"colsx",
                b"linebetcol",
                b"colno",
                b"colw",
                b"colsr",
                b"column",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/column-section-break-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Left segment.", "Right segment."],
            must_not_leak: &[
                b"cols2",
                b"sbkcol",
                b"\\sect",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/tab-alignment-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Left label",
                "9",
                "Decimal value",
                "12.3",
                "Center label",
                "C",
                "Default tab",
                "width",
                "Bar tab",
                "text",
            ],
            must_not_leak: &[
                b"tqr",
                b"tqdec",
                b"tqc",
                b"tldot",
                b"tlmdot",
                b"deftab360",
                b"tx1440",
                b"tx2160",
                b"tx2880",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" S"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/tab-leader-spacing-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["A", "X"],
            must_not_leak: &[
                b"tqr",
                b"tldot",
                b"tx1440",
                b"tx2160",
                b"tx2880",
                b"tx3600",
                b"tx4320",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/tab-leader-variants-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Dot A",
                "Hyphen A",
                "Underline A",
                "Thick A",
                "Double A",
                "Middle A",
                "Equals A",
                "X",
            ],
            must_not_leak: &[
                b"tqr",
                b"tldot",
                b"tlhyph",
                b"tlul",
                b"tlth",
                b"tluldb",
                b"tlmdot",
                b"tleq",
                b"tx2880",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/legacy-textbox-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before legacy box.", "Legacy box text", "After legacy box."],
            must_not_leak: &[
                b"dobx",
                b"dobxcolumn",
                b"doby",
                b"dobypara",
                b"dodhgt",
                b"dpxsize",
                b"dpysize",
                b"dptxbx",
                b"dptxbxtext",
                b"objemb",
                b"objclass",
                b"objdata",
                b"Word.Picture.8",
                b"414243",
                b"dpptx",
                b"hidden-legacy-textbox-coordinate-payload",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: OLE object before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shading-patterns-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Patterned paragraph.",
                "Row shade",
                "Cell shade",
                "After shading.",
            ],
            must_not_leak: &[
                b"colortbl",
                b"red255",
                b"green0",
                b"blue255",
                b"cbpat",
                b"cfpat",
                b"shading2500",
                b"bghoriz",
                b"trcbpat",
                b"trcfpat",
                b"trshdng",
                b"trbgvert",
                b"clcbpat",
                b"clcfpat",
                b"clshdng",
                b"clbgdkdcross",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/paragraph-borders-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before paragraph borders.",
                "Boxed paragraph.",
                "Bar paragraph.",
                "Between first.",
                "Between second.",
                "After paragraph borders.",
            ],
            must_not_leak: &[
                b"brdrbar",
                b"brdrbtw",
                b"brdrdash",
                b"brdrs",
                b"brdrw",
                b"brdrcf",
                b"colortbl",
                b"red255",
                b"blue255",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b" S"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/table-borders-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before table borders.",
                "Top left",
                "Diagonal cell",
                "Right cell",
                "After table borders.",
            ],
            must_not_leak: &[
                b"colortbl",
                b"red255",
                b"blue255",
                b"trowd",
                b"trbrdrt",
                b"trbrdrh",
                b"trbrdrv",
                b"clbrdrl",
                b"clbrdrt",
                b"cldglu",
                b"cldgll",
                b"brdrdb",
                b"brdrdash",
                b"brdrw",
                b"brdrcf",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "row-level table borders ignored to match Word print behavior",
            ],
        },
        ReferenceFixture {
            input: "fixtures/carried-table-row-definitions-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before carried table.",
                "Alpha",
                "Beta",
                "Gamma",
                "Delta",
                "After carried table.",
            ],
            must_not_leak: &[
                b"trowd",
                b"trgaph",
                b"trbrdrb",
                b"clbrdrl",
                b"clbrdrt",
                b"clbrdrb",
                b"clbrdrr",
                b"brdrs",
                b"brdrw",
                b"cellx",
                b"pard",
                b"intbl",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
                b"/URI",
            ],
            must_contain_pdf: &[b" S"],
            must_emit_diagnostics: &[
                "row-level table borders ignored to match Word print behavior",
            ],
        },
        ReferenceFixture {
            input: "fixtures/intbl-zero-body-flow-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before table-state reset.",
                "Table cell",
                "Outside table body.",
                "After table-state reset.",
            ],
            must_not_leak: &[
                b"trowd",
                b"cellx",
                b"pard",
                b"intbl0",
                b"intbl",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/table-merged-cells-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before merged table.",
                "Horizontal merged",
                "Right top",
                "Vertical merged",
                "Middle",
                "Right middle",
                "Bottom middle",
                "Right bottom",
                "After merged table.",
            ],
            must_not_leak: &[
                b"colortbl",
                b"red220",
                b"trowd",
                b"trgaph",
                b"trrh",
                b"clmgf",
                b"clmrg",
                b"clvmgf",
                b"clvmrg",
                b"clcbpat",
                b"clbrdrt",
                b"clbrdrl",
                b"clbrdrb",
                b"clbrdrr",
                b"brdrw",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/nested-table-flatten-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before nested table.",
                "Outer before",
                "Inner A",
                "Inner B",
                "Outer after",
                "After nested table.",
            ],
            must_not_leak: &[
                b"trowd",
                b"itap",
                b"nesttableprops",
                b"nestcell",
                b"nestrow",
                b"rtlrow",
                b"ltrrow",
                b"trpaddfl",
                b"trpaddl",
                b"trspdfl",
                b"trspdl",
                b"clpadfl",
                b"clpadl",
                b"clspdfl",
                b"clspdl",
                b"tphmrg",
                b"tposx",
                b"tpvpg",
                b"tposyin",
                b"tdfrmtxtLeft",
                b"tabsnoovrlp",
                b"cellx",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "nested table normalized into its containing cell; nested borders and layout use a bounded passive approximation",
            ],
        },
        ReferenceFixture {
            input: "fixtures/old-style-list-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before old list.",
                "Indented decimal item",
                "Upper roman item",
                "After old list.",
            ],
            must_not_leak: &[
                b"pnindent",
                b"pnhang",
                b"pnsp",
                b"pnstart",
                b"pndec",
                b"pnucrm",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/section-number-passive.rtf",
            expected_pages: 3,
            must_preserve_text: &["Part 1", "Part 2", "Part 3"],
            must_not_leak: &[
                b"sectnum",
                b"fldinst",
                b"SECTION",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/picture-scaling-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before scaled image.", "After scaled image."],
            must_not_leak: &[
                b"pngblip",
                b"picw2",
                b"pich1",
                b"picwgoal",
                b"pichgoal",
                b"picscalex",
                b"picscaley",
                b"piccropl",
                b"piccropt",
                b"piccropr",
                b"piccropb",
                b"IHDR",
                b"IDAT",
                b"IEND",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b"/Subtype /Image"],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/picture-adjustments-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before adjusted images.",
                "Grayscale image:",
                "Bilevel image:",
                "Tone image:",
                "After adjusted images.",
            ],
            must_not_leak: &[
                b"pngblip",
                b"picprop",
                b"pictureGray",
                b"pictureBiLevel",
                b"pictureBrightness",
                b"pictureContrast",
                b"picwgoal",
                b"pichgoal",
                b"IHDR",
                b"IDAT",
                b"IEND",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[b"/Subtype /Image"],
            must_emit_diagnostics: &[
                "PNG picture grayscale property rendered as bounded passive image pixels",
                "PNG picture bilevel property rendered as bounded passive image pixels",
                "PNG picture brightness/contrast property rendered as bounded passive image pixels",
            ],
        },
        ReferenceFixture {
            input: "fixtures/jpeg-trailing-payload-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before JPEG payload.", "After JPEG payload."],
            must_not_leak: &[
                b"jpegblip",
                b"TRAILING-JPEG-AFTER-EOI",
                b"/DCTDecode",
                b"/Subtype /Image",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "malformed JPEG picture suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/associated-character-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before associated text.",
                "Associated styled text",
                "Plain after associated text.",
            ],
            must_not_leak: &[
                b"fonttbl",
                b"colortbl",
                b"loch",
                b"acf1",
                b"aul",
                b"aexpnd4",
                b"aup6",
                b"objdata",
                b"414243",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
                b"/URI",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: OLE object before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/font-family-hints-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Roman family text.",
                "Modern family text.",
                "Theme heading text.",
                "Theme body text.",
            ],
            must_not_leak: &[
                b"fonttbl",
                b"froman",
                b"fmodern",
                b"flomajor",
                b"fhiminor",
                b"Mystery Serif",
                b"Mystery Mono",
                b"Mystery Heading",
                b"Mystery Body",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/font-charset-greek-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before Greek charset.", "Greek", "After Greek charset."],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"fcharset161",
                b"Times New Roman Greek",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/font-charset-cyrillic-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before Cyrillic charset.",
                "Cyrillic",
                "After Cyrillic charset.",
            ],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"fcharset204",
                b"Times New Roman Cyr",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/font-charset-central-european-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before Central European charset.",
                "Central European",
                "After Central European charset.",
            ],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"fcharset238",
                b"Times New Roman CE",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/font-charset-turkish-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before Turkish charset.",
                "Turkish",
                "After Turkish charset.",
            ],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"fcharset162",
                b"Times New Roman Tur",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/font-charset-baltic-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before Baltic charset.", "Baltic", "After Baltic charset."],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"fcharset186",
                b"Times New Roman Baltic",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/font-code-page-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before font code pages.",
                "IBM cafe \u{201a} box \u{00b3}",
                "Mac quote \u{00d2}Hello\u{00d3}",
                "After font code pages.",
            ],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"cpg437",
                b"cpg10000",
                b"Terminal",
                b"Mac Face",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/font-charset-legacy-code-page-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before legacy charset code pages.",
                "Mac quote",
                "OEM cafe",
                "After legacy charset code pages.",
            ],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"fcharset77",
                b"fcharset255",
                b"Mac Face",
                b"Oem Face",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/zero-width-formatting-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before zero width.",
                "A",
                "B",
                "C",
                "D",
                "E",
                "L",
                "R",
                "Visible after zero width.",
            ],
            must_not_leak: &[
                b"zwbo",
                b"zwnbo",
                b"zwnj",
                b"zwj",
                b"ltrmark",
                b"rtlmark",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/symbol-font-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before Symbol font.", "Symbol glyphs", "After Symbol font."],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"fcharset2",
                b"Symbol;",
                b"'b7",
                b"'a3",
                b"'b1",
                b"'d6",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/symbol-field-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before Symbol field.",
                "Field value A rendered passively.",
                "After Symbol field.",
            ],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"fldinst",
                b"SYMBOL",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
                b"/URI",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/pntext-symbol-list-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before pntext list.",
                "Bullet item",
                "Roman item",
                "After pntext list.",
            ],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"fcharset2",
                b"Symbol;",
                b"pntext",
                b"pntxtb",
                b"pntxta",
                b"pnlvlblt",
                b"pnucrm",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
                b"/URI",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wingdings-checkbox-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before Wingdings checkboxes.",
                "Box states:",
                "Field states:",
                "After Wingdings checkboxes.",
            ],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"Wingdings",
                b"fldinst",
                b"SYMBOL",
                b"ZapfDingbats",
                b"'a3",
                b"'fe",
                b"'fc",
                b"'fb",
                b"'fd",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
                b"/URI",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wingdings2-checkbox-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before Wingdings 2 checkboxes.",
                "Box states:",
                "Field state:",
                "After Wingdings 2 checkboxes.",
            ],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"Wingdings 2",
                b"fldinst",
                b"SYMBOL",
                b"ZapfDingbats",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
                b"/URI",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/wingdings3-arrows-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before Wingdings 3 arrows.",
                "Arrow states:",
                "Field arrow:",
                "After Wingdings 3 arrows.",
            ],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"Wingdings 3",
                b"fldinst",
                b"SYMBOL",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
                b"/URI",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/webdings-checkbox-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before Webdings checkboxes.",
                "Box states:",
                "Field state:",
                "After Webdings checkboxes.",
            ],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"Webdings",
                b"fldinst",
                b"SYMBOL",
                b"ZapfDingbats",
                b"'3f",
                b"'61",
                b"'63",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
                b"/URI",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/segoe-ui-symbol-checkbox-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before Segoe UI Symbol checkboxes.",
                "Label",
                "After Segoe UI Symbol checkboxes.",
            ],
            must_not_leak: &[
                b"ansicpg1252",
                b"fonttbl",
                b"Segoe UI Symbol",
                b"u9744",
                b"u9745",
                b"u9746",
                b"u10003",
                b"u10007",
                b"ZapfDingbats",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
                b"/URI",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/form-checkbox-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before form checkboxes.",
                "Checked",
                "item.",
                "Unchecked",
                "After form checkboxes.",
            ],
            must_not_leak: &[
                b"FORMCHECKBOX",
                b"formfield",
                b"fftype1",
                b"ffname",
                b"HiddenChecked",
                b"HiddenUnchecked",
                b"ffdefres",
                b"ffres",
                b"ffentrymcr",
                b"launch.exe",
                b"datafield",
                b"414243",
                b"444546",
                b"ZapfDingbats",
                b"/AcroForm",
                b"/Widget",
                b"/AA",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/Annots",
                b"/URI",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "form checkbox is editor-only and omitted from passive print output",
                "rendering passive form field FORMCHECKBOX without creating PDF form actions",
            ],
        },
        ReferenceFixture {
            input: "fixtures/form-text-dropdown-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before form text and dropdown fields.",
                "Stored text Visible stored value",
                "Default text  empty text",
                "Dropdown",
                "Visible after form text and dropdown fields.",
            ],
            must_not_leak: &[
                b"fldinst",
                b"fldrslt",
                b"FORMTEXT",
                b"FORMDROPDOWN",
                b"formfield",
                b"fftype",
                b"ffname",
                b"ffdeftext",
                b"ffdefres",
                b"ffres",
                b"ffl",
                b"HiddenStored",
                b"HiddenDefault",
                b"HiddenText",
                b"EmptyText",
                b"HiddenDrop",
                b"First choice",
                b"launch.exe",
                b"https://example.com",
                b"datafield",
                b"414243",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Widget",
                b"/AA",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering stored result for passive form field FORMTEXT without creating PDF form actions",
                "form field FORMTEXT is editor-only and omitted from passive print output",
                "rendering passive form field FORMTEXT without creating PDF form actions",
                "form field FORMDROPDOWN is editor-only and omitted from passive print output",
                "rendering passive form field FORMDROPDOWN without creating PDF form actions",
            ],
        },
        ReferenceFixture {
            input: "fixtures/active-control-fields-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before active controls.",
                "Button [Field removed: no passive result]",
                "Checkbox [Field removed: no passive result]",
                "Input [Field removed: no passive result]",
                "Select [Field removed: no passive result]",
                "Visible after active controls.",
            ],
            must_not_leak: &[
                b"CONTROL",
                b"HTMLCHECKBOX",
                b"HTMLINPUT",
                b"HTMLSELECT",
                b"Forms.CommandButton",
                b"Hidden caption",
                b"secret-name",
                b"password",
                b"value=",
                b"Secret option",
                b"fldinst",
                b"/AcroForm",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active control field CONTROL rendered as passive placeholder without creating active form or control content",
                "active control field HTMLCHECKBOX rendered as passive placeholder without creating active form or control content",
                "active control field HTMLINPUT rendered as passive placeholder without creating active form or control content",
                "active control field HTMLSELECT rendered as passive placeholder without creating active form or control content",
            ],
        },
        ReferenceFixture {
            input: "fixtures/document-protection-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible protected body.",
            ],
            must_not_leak: &[
                b"passwordhash",
                b"DEADBEEF",
                b"AABBCC",
                b"formprot",
                b"revprot",
                b"annotprot",
                b"/Encrypt",
                b"/Perms",
                b"/AcroForm",
                b"/Widget",
                b"/AA",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &["document protection password hash stripped"],
        },
        ReferenceFixture {
            input: "fixtures/mailmerge-object-metadata-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before metadata.",
                "Mail merge  after merge.",
                "Object meta  middle  after object meta.",
                "Visible after metadata.",
            ],
            must_not_leak: &[
                b"mailmerge",
                b"mmconnectstr",
                b"mmdatasource",
                b"mmquery",
                b"mmodsoudl",
                b"Provider=SQLOLEDB",
                b"Password=secret",
                b"Data Source",
                b"contacts.mdb",
                b"SELECT *",
                b"example.com",
                b"source.udl",
                b"objclass",
                b"objname",
                b"objalias",
                b"objtopic",
                b"HiddenClass",
                b"HiddenName",
                b"HiddenAlias",
                b"HiddenTopic",
                b"objdata",
                b"414243",
                b"444546",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: mail merge data source before safe model normalization",
                "active content removed: mail merge data source in metadata before safe model normalization",
                "active content removed: object payload in metadata before safe model normalization",
                "active content removed: object metadata before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/private-metadata-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before private metadata.",
                "Storage  after storage.",
                "Metadata  after metadata.",
                "Unknown  after unknown.",
                "Visible after private metadata.",
            ],
            must_not_leak: &[
                b"Hidden private payload",
                b"Hidden info payload",
                b"Hidden skipped payload",
                b"object",
                b"objdata",
                b"414243",
                b"504b0304",
                b"4d5a9000",
                b"HYPERLINK",
                b"https://example.com/private",
                b"https://example.com/info",
                b"Hidden link",
                b"Hidden info link",
                b"fldinst",
                b"fldrslt",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: private metadata before safe model normalization",
                "active content removed: private metadata in metadata before safe model normalization",
                "active content removed: private metadata in skipped destination before safe model normalization",
                "active content removed: object payload in metadata before safe model normalization",
                "active content removed: object payload in skipped destination before safe model normalization",
                "active content removed: field instruction in metadata before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/review-bookmark-metadata-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before review metadata.",
                "Comment after comment.",
                "Bookmark visible after bookmark.",
                "Revision  visible after Inserted text.",
                "Visible after review metadata.",
            ],
            must_not_leak: &[
                b"annotation",
                b"Hidden comment",
                b"Hidden result",
                b"Hidden link",
                b"HYPERLINK",
                b"https://example.com/comment",
                b"SecretBookmark",
                b"bkmkstart",
                b"bkmkend",
                b"Deleted text",
                b"deleted",
                b"revised",
                b"revauth",
                b"revdttm",
                b"insrsid",
                b"objdata",
                b"414243",
                b"fldinst",
                b"fldrslt",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: annotation metadata before safe model normalization",
                "active content removed: object payload in metadata before safe model normalization",
                "active content removed: field instruction in metadata before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/hidden-animation-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before hidden and animation.",
                "Hidden Shown text after hidden.",
                "Animation Animated text Static text.",
                "Visible after hidden and animation.",
            ],
            must_not_leak: &[
                b"Hidden text",
                b"INCLUDEPICTURE",
                b"https://example.com/hidden.png",
                b"objdata",
                b"414243",
                b"shpinst",
                b"pFragments",
                b"hidden-payload",
                b"fldinst",
                b"animtext",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: OLE object before safe model normalization",
                "hidden shape stripped before safe model normalization",
                "character animation stripped for passive static PDF output",
            ],
        },
        ReferenceFixture {
            input: "fixtures/font-template-metadata-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible Arial text.",
                "Visible Courier text.",
                "Visible after font and template metadata.",
            ],
            must_not_leak: &[
                b"fontemb",
                b"fontfile",
                b"HOSTILE-FONT-PAYLOAD",
                b"HIDDEN-METADATA-FONT",
                b"Hidden title",
                b"Hidden link",
                b"HYPERLINK",
                b"https://example.com",
                b"template.dotm",
                b"t.dotm",
                b"objdata",
                b"414243",
                b"fldinst",
                b"fldrslt",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: embedded font payload before safe model normalization",
                "active content removed: embedded font payload in metadata before safe model normalization",
                "active content removed: external template in metadata before safe model normalization",
                "active content removed: field instruction in metadata before safe model normalization",
                "active content removed: external template before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/macro-script-metadata-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible before macro script metadata.",
                "Macro  after macro.",
                "Script  after script.",
                "VBA  after vba.",
                "Metadata  after metadata.",
                "Visible after macro script metadata.",
            ],
            must_not_leak: &[
                b"macros",
                b"vbaproject",
                b"activex",
                b"launch.exe",
                b"<script>",
                b"414243",
                b"Hidden VBA payload",
                b"Hidden title",
                b"Forms.CommandButton",
                b"https://example.com",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Widget",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: macro/script payload before safe model normalization",
                "active content removed: macro/script payload in metadata before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/html-xml-metadata-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible HTML body after html.",
                "Before XML tagged",
                "After XML.",
                "Visible after HTML XML metadata.",
            ],
            must_not_leak: &[
                b"fromhtml",
                b"htmlrtf",
                b"htmltag",
                b"htmlbase",
                b"Hidden HTML",
                b"onclick",
                b"launch.exe",
                b"<script>",
                b"https://example.com/base",
                b"datastore",
                b"themedata",
                b"colorschememapping",
                b"xmlopen",
                b"xmlclose",
                b"xmlattrname",
                b"xmlattrvalue",
                b"secret",
                b"objdata",
                b"414243",
                b"504b0304",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: object payload in skipped destination before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/word-metadata-session-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Word metadata text.",
                "Session layout text.",
                "Before tagged",
                "After.",
            ],
            must_not_leak: &[
                b"adeflang",
                b"nobrkwrptbl",
                b"sprstsp",
                b"doctype",
                b"oldas",
                b"donotembedsysfont",
                b"donotembedlingdata",
                b"trackmoves",
                b"trackformatting",
                b"validatexml",
                b"showxmlerrors",
                b"saveinvalidxml",
                b"usenormstyforlist",
                b"deflang",
                b"nouicompat",
                b"rsid",
                b"rsidtbl",
                b"insrsid",
                b"delrsid",
                b"xmlnstbl",
                b"xmlns",
                b"schemas.example",
                b"datastore",
                b"themedata",
                b"colorschememapping",
                b"xmlopen",
                b"xmlclose",
                b"xmlattrname",
                b"xmlattrvalue",
                b"secret",
                b"lang1033",
                b"loch",
                b"hich",
                b"dbch",
                b"ltrch",
                b"kerning",
                b"hyphpar",
                b"adjustright",
                b"vertdoc",
                b"wraptrsp",
                b"sprsspbf",
                b"sprsbsp",
                b"object",
                b"objdata",
                b"414243",
                b"444546",
                b"504b0304",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "character kerning rendered as bounded passive pair spacing",
                "paragraph hyphenation disabled",
                "vertical document layout rendered through bounded passive horizontal fallback",
                "Word typography compatibility option interpreted through bounded passive layout",
                "active content removed: object payload in skipped destination before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/stylesheet-metadata-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Heading",
                "linked text",
                "Visible after stylesheet metadata.",
            ],
            must_not_leak: &[
                b"stylesheet",
                b"ssemihidden",
                b"sqformat",
                b"spriority",
                b"slink",
                b"slocked",
                b"sautoupd",
                b"styrsid",
                b"latentstyles",
                b"lsdstimax",
                b"lsdlockeddef",
                b"lsdsemihiddendef",
                b"lsdlockedexcept",
                b"Heading 1",
                b"Linked Char",
                b"object",
                b"objdata",
                b"414243",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "active content removed: object payload in metadata before safe model normalization",
            ],
        },
        ReferenceFixture {
            input: "fixtures/structural-metadata-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible body.",
                "Body Marked text visible Marked text hidden [Field removed: no passive result] style [Field removed: no passive result]",
            ],
            must_not_leak: &[
                b"Hidden font payload",
                b"Hidden style payload",
                b"HiddenFontMark",
                b"HiddenStyleMark",
                b"Hidden title",
                b"[Shape skipped",
                b"Image skipped",
                b"unsupported picture",
                b"HYPERLINK",
                b"INCLUDEPICTURE",
                b"hidden.example",
                b"payload.png",
                b"fldinst",
                b"fonttbl",
                b"stylesheet",
                b"colortbl",
                b"bkmkstart",
                b"bkmkend",
                b"shapeType",
                b"pFragments",
                b"hidden-metadata-shape-payload",
                b"shppict",
                b"nonshppict",
                b"pict",
                b"pngblip",
                b"wmetafile",
                b"2f4a617661536372697074",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/word-grid-outline-metadata-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Visible metadata.", "Styled visible metadata."],
            must_not_leak: &[
                b"outlinelevel",
                b"cgrid",
                b"stylesheet",
                b"Heading",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/stylesheet-semantics-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Inherited styled text",
                "Heading text",
                "Body text",
                "Right Bold only",
                "Direct Direct and styled",
            ],
            must_not_leak: &[
                b"stylesheet",
                b"sbasedon",
                b"snext",
                b"Child",
                b"Heading;",
                b"Body;",
                b"cs5",
                b"li1440",
                b"Emphasis",
                b"fonttbl",
                b"Courier New",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/word-layout-compatibility-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Visible compatibility text.",
                "Follow-up compatibility paragraph.",
            ],
            must_not_leak: &[
                b"themelang",
                b"dghspace",
                b"dgvspace",
                b"dghorigin",
                b"dgvorigin",
                b"dghshow",
                b"dgvshow",
                b"viewbksp",
                b"viewnobound",
                b"formdisp",
                b"rempersonalinfo",
                b"jexpand",
                b"jcompress",
                b"jclisttab",
                b"asianbrkrule",
                b"dxfrtext",
                b"dfrmtxtx",
                b"dfrmtxty",
                b"absw",
                b"absh",
                b"faauto",
                b"twoinone",
                b"horzvert",
                b"aspalpha",
                b"aspnum",
                b"nogrowautofit",
                b"lytexcttp",
                b"lytprtmet",
                b"noextrasprl",
                b"notcvasp",
                b"notvatxbx",
                b"expshrtn",
                b"useltbaln",
                b"htmautsp",
                b"nooverflow",
                b"wpjst",
                b"wpbrdr",
                b"otblrul",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "Japanese text justification interpreted through bounded passive line layout",
                "Asian line-breaking rule interpreted through bounded passive Unicode line layout",
                "East Asian font alignment interpreted through bounded passive font metrics",
                "East Asian inline layout rendered through bounded passive horizontal fallback",
                "Word auto-spacing compatibility interpreted through bounded passive text layout",
                "paragraph wrap distance interpreted through bounded passive flow layout",
                "paragraph frame dimensions interpreted through bounded passive flow layout",
                "Word typography compatibility option interpreted through bounded passive layout",
                "table autofit growth disabled through bounded passive authored widths",
            ],
        },
        ReferenceFixture {
            input: "fixtures/section-pages-passive.rtf",
            expected_pages: 3,
            must_preserve_text: &[
                "First section pages 2",
                "First section continues.",
                "Second section pages 1",
            ],
            must_not_leak: &[
                b"fldinst",
                b"SECTIONPAGES",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/header-footer-variants-passive.rtf",
            expected_pages: 3,
            must_preserve_text: &[
                "First header",
                "Odd header",
                "First footer",
                "Odd footer",
                "Page one body.",
                "Page two body.",
                "Page three body.",
            ],
            must_not_leak: &[
                b"headerf",
                b"headerl",
                b"headerr",
                b"footerf",
                b"footerl",
                b"footerr",
                b"titlepg",
                b"Even header",
                b"Even footer",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/header-footer-distance-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Header", "Footer", "Body"],
            must_not_leak: &[
                b"headery",
                b"footery",
                b"/Action",
                b"/Annots",
                b"/URI",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/background-shape-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Body over passive background."],
            must_not_leak: &[
                b"background",
                b"Hidden background text",
                b"shpinst",
                b"shapeType",
                b"fillColor",
                b"pFragments",
                b"hostile-background-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/shape-z-order-text-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &[
                "Before z-order shape.",
                "Layered Box",
                "After z-order shape.",
            ],
            must_not_leak: &[
                b"shpinst",
                b"shpz",
                b"shapeType",
                b"pFragments",
                b"hidden-z-order-reference-payload",
                b"shptxt",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "shape z-order rendered through bounded passive drawing order",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-arrowheads-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before arrow line.", "After arrow line."],
            must_not_leak: &[
                b"dpline",
                b"dpx360",
                b"dpy480",
                b"dpxsize1440",
                b"dpysize720",
                b"lineStartArrowhead",
                b"lineEndArrowhead",
                b"triangle",
                b"pFragments",
                b"hostile-arrowhead-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/shape-line-dashing-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before dashed line.", "After dashed line."],
            must_not_leak: &[
                b"dpline",
                b"dpx360",
                b"dpy480",
                b"dpxsize1440",
                b"dpysize720",
                b"lineDashing",
                b"dashDot",
                b"pFragments",
                b"hostile-line-dashing-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/shape-line-cap-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before capped line.", "After capped line."],
            must_not_leak: &[
                b"dpline",
                b"dpx360",
                b"dpy480",
                b"dpxsize1440",
                b"dpysize720",
                b"lineEndCap",
                b"round",
                b"pFragments",
                b"hostile-line-cap-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-line-join-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before joined line.", "After joined line."],
            must_not_leak: &[
                b"dppolyline",
                b"dpptx360",
                b"dppty480",
                b"dpptx1440",
                b"dppty1200",
                b"lineWidth",
                b"lineJoinStyle",
                b"round",
                b"pFragments",
                b"hostile-line-join-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "malformed legacy polyline vertex count suppressed for Word-compatible passive rendering",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-pattern-fill-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before pattern fill.", "After pattern fill."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"fillType",
                b"msoFillPattern",
                b"fillColor",
                b"pFragments",
                b"hostile-pattern-fill-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-gradient-fill-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before gradient fill.", "After gradient fill."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"fillType",
                b"msofillShade",
                b"fillColor",
                b"fillBackColor",
                b"16711680",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[],
        },
        ReferenceFixture {
            input: "fixtures/shape-shadow-opacity-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before shadow shape.", "After shadow shape."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"fillColor",
                b"fShadow",
                b"shadowColor",
                b"shadowOpacity",
                b"32768",
                b"pFragments",
                b"hostile-shadow-opacity-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-fill-fore-color-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before fill color.", "After fill color."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"fillForeColor",
                b"pFragments",
                b"hostile-fill-fore-color-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-line-fore-color-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before stroke color.", "After stroke color."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"lineForeColor",
                b"pFragments",
                b"hostile-line-fore-color-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-fill-back-color-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before back fill.", "After back fill."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"fillBackColor",
                b"pFragments",
                b"hostile-fill-back-color-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-line-back-color-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before back stroke.", "After back stroke."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"lineBackColor",
                b"pFragments",
                b"hostile-line-back-color-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-fill-flag-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before fill flag.", "After fill flag."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"fillColor",
                b"fFilled",
                b"pFragments",
                b"hostile-fill-flag-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-line-flag-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before line flag.", "After line flag."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"fLine",
                b"pFragments",
                b"hostile-line-flag-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-disabled-fill-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before disabled fill.", "After disabled fill."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"fFilled",
                b"fillColor",
                b"pFragments",
                b"hostile-disabled-fill-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-disabled-line-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before disabled line.", "After disabled line."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"fLine",
                b"lineWidth",
                b"pFragments",
                b"hostile-disabled-line-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "fixtures/shape-hidden-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before hidden shape.", "After hidden shape."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"fHidden",
                b"fillColor",
                b"lineWidth",
                b"pFragments",
                b"hostile-hidden-shape-reference-payload",
                b"Hidden shape text must not render.",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &["hidden shape stripped before safe model normalization"],
        },
        ReferenceFixture {
            input: "fixtures/shape-flip-passive.rtf",
            expected_pages: 1,
            must_preserve_text: &["Before flipped shape.", "After flipped shape."],
            must_not_leak: &[
                b"shpinst",
                b"shapeType",
                b"fFlipH",
                b"fFlipV",
                b"lineColor",
                b"lineWidth",
                b"pFragments",
                b"hostile-flipped-shape-reference-payload",
                b"[Shape skipped",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
                b"/RichMedia",
                b"/AcroForm",
                b"/Annots",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering bounded passive static drawing shape and stripping unsupported/active drawing properties",
            ],
        },
        ReferenceFixture {
            input: "docs/sample.rtf",
            expected_pages: 2,
            must_preserve_text: &[
                "It is an example test rtf-file to RTF2XML bean for testing",
                "Simple table",
                "Here are some special characters",
                "At last you can see an image",
            ],
            must_not_leak: &[
                b"objdata",
                b"Word.Picture.8",
                b"METAFILEPICT",
                b"shppict",
                b"shprslt",
                b"wmetafile8",
                b"/JavaScript",
                b"/EmbeddedFile",
                b"/Launch",
                b"/OpenAction",
            ],
            must_contain_pdf: &[],
            must_emit_diagnostics: &[
                "rendering shape picture result with bounded passive shape frame",
                "ignoring duplicate embedded object alternate after passive shape result",
                "active content removed: object metadata in skipped destination",
                "ignoring duplicate shape result fallback after passive primary shape result",
            ],
        },
    ]
}

fn decoded_pdf_text(pdf: &PdfDocument) -> String {
    let mut output = String::new();
    for page_id in pdf.get_pages().values() {
        let content = pdf
            .get_and_decode_page_content(*page_id)
            .expect("page content should decode");
        output.push_str(&content_text(&content));
        output.push('\n');
    }
    output
}

fn normalize_pdf_contract_text(text: &str) -> String {
    text.split_whitespace().collect::<Vec<_>>().join(" ")
}

fn pdf_contract_text_contains(rendered: &str, expected: &str) -> bool {
    rendered.contains(expected)
        || rendered
            .chars()
            .filter(|ch| !ch.is_whitespace())
            .collect::<String>()
            .contains(
                &expected
                    .chars()
                    .filter(|ch| !ch.is_whitespace())
                    .collect::<String>(),
            )
}

fn content_text(content: &Content) -> String {
    let mut text = String::new();
    for operation in &content.operations {
        match operation.operator.as_ref() {
            "Tj" | "'" | "\"" => {
                for operand in &operation.operands {
                    if let Ok(bytes) = operand.as_str() {
                        text.push_str(&decode_pdf_text_bytes(bytes));
                    }
                }
            }
            "TJ" => {
                for operand in &operation.operands {
                    if let Ok(items) = operand.as_array() {
                        for item in items {
                            if let Ok(bytes) = item.as_str() {
                                text.push_str(&decode_pdf_text_bytes(bytes));
                            }
                        }
                    }
                }
            }
            _ => {}
        }
    }
    text
}

fn decode_pdf_text_bytes(bytes: &[u8]) -> String {
    if bytes_look_like_utf16be_cids(bytes) {
        let utf16 = bytes
            .chunks_exact(2)
            .map(|chunk| u16::from_be_bytes([chunk[0], chunk[1]]))
            .collect::<Vec<_>>();
        if let Ok(decoded) = String::from_utf16(&utf16) {
            return decoded;
        }
    }
    String::from_utf8_lossy(bytes).into_owned()
}

fn bytes_look_like_utf16be_cids(bytes: &[u8]) -> bool {
    if bytes.len() < 2 || bytes.len() % 2 != 0 {
        return false;
    }
    let chunks = bytes.len() / 2;
    let zero_high_bytes = bytes.chunks_exact(2).filter(|chunk| chunk[0] == 0).count();
    zero_high_bytes * 2 >= chunks
}

fn json_usize_for_key(object: &str, key: &str) -> usize {
    let marker = format!("\"{key}\":");
    let start = object
        .find(&marker)
        .unwrap_or_else(|| panic!("missing numeric key {key}"))
        + marker.len();
    let value = object[start..]
        .trim_start()
        .chars()
        .take_while(|ch| ch.is_ascii_digit())
        .collect::<String>();
    value
        .parse()
        .unwrap_or_else(|error| panic!("invalid numeric key {key}: {error}"))
}

fn json_optional_usize_for_key(object: &str, key: &str) -> Option<usize> {
    let marker = format!("\"{key}\":");
    let start = object.find(&marker)? + marker.len();
    let value = object[start..]
        .trim_start()
        .chars()
        .take_while(|ch| ch.is_ascii_digit())
        .collect::<String>();
    (!value.is_empty()).then(|| {
        value
            .parse()
            .unwrap_or_else(|error| panic!("invalid numeric key {key}: {error}"))
    })
}

fn json_string_for_key(object: &str, key: &str) -> String {
    let marker = format!("\"{key}\":");
    let start = object
        .find(&marker)
        .unwrap_or_else(|| panic!("missing string key {key}"))
        + marker.len();
    let value = object[start..].trim_start();
    let (decoded, _) = parse_json_string(value)
        .unwrap_or_else(|| panic!("invalid JSON string for key {key}: {value:?}"));
    decoded
}

fn json_nullable_string_for_key(object: &str, key: &str) -> Option<String> {
    let marker = format!("\"{key}\":");
    let start = object
        .find(&marker)
        .unwrap_or_else(|| panic!("missing nullable string key {key}"))
        + marker.len();
    let value = object[start..].trim_start();
    if value.starts_with("null") {
        return None;
    }
    let (decoded, _) = parse_json_string(value)
        .unwrap_or_else(|| panic!("invalid JSON nullable string for key {key}: {value:?}"));
    Some(decoded)
}

fn json_string_array_for_key(object: &str, key: &str) -> Vec<String> {
    let array = json_array_for_key(object, key);
    let mut values = Vec::new();
    let mut rest = array.trim();
    while !rest.is_empty() {
        rest = rest.trim_start();
        if let Some(after_comma) = rest.strip_prefix(',') {
            rest = after_comma;
            continue;
        }
        let Some((value, consumed)) = parse_json_string(rest) else {
            panic!("invalid JSON string array item for key {key}: {rest:?}");
        };
        values.push(value);
        rest = &rest[consumed..];
    }
    values
}

fn json_optional_string_array_for_key(object: &str, key: &str) -> Option<Vec<String>> {
    object
        .contains(&format!("\"{key}\":"))
        .then(|| json_string_array_for_key(object, key))
}

fn json_array_for_key<'a>(object: &'a str, key: &str) -> &'a str {
    let marker = format!("\"{key}\":");
    let start = object
        .find(&marker)
        .unwrap_or_else(|| panic!("missing array key {key}"))
        + marker.len();
    let after_marker = object[start..].trim_start();
    let open = after_marker
        .find('[')
        .unwrap_or_else(|| panic!("missing array open for key {key}"));
    let array_start = open + 1;
    let mut depth = 0usize;
    let mut in_string = false;
    let mut escaped = false;
    for (idx, ch) in after_marker[array_start..].char_indices() {
        if in_string {
            if escaped {
                escaped = false;
            } else if ch == '\\' {
                escaped = true;
            } else if ch == '"' {
                in_string = false;
            }
            continue;
        }
        match ch {
            '"' => in_string = true,
            '[' => depth += 1,
            ']' if depth == 0 => return &after_marker[array_start..array_start + idx],
            ']' => depth -= 1,
            _ => {}
        }
    }
    panic!("unterminated array for key {key}")
}

fn split_json_objects(array: &str) -> Vec<&str> {
    let mut objects = Vec::new();
    let mut object_start = None;
    let mut depth = 0usize;
    let mut in_string = false;
    let mut escaped = false;
    for (idx, ch) in array.char_indices() {
        if in_string {
            if escaped {
                escaped = false;
            } else if ch == '\\' {
                escaped = true;
            } else if ch == '"' {
                in_string = false;
            }
            continue;
        }
        match ch {
            '"' => in_string = true,
            '{' => {
                if depth == 0 {
                    object_start = Some(idx);
                }
                depth += 1;
            }
            '}' => {
                depth = depth.checked_sub(1).expect("unexpected JSON object close");
                if depth == 0 {
                    let start = object_start.take().expect("missing JSON object start");
                    objects.push(&array[start..=idx]);
                }
            }
            _ => {}
        }
    }
    objects
}

fn parse_json_string(value: &str) -> Option<(String, usize)> {
    let mut chars = value.char_indices();
    if chars.next()?.1 != '"' {
        return None;
    }
    let mut output = String::new();
    let mut escaped = false;
    for (idx, ch) in chars {
        if escaped {
            let decoded = match ch {
                '"' => '"',
                '\\' => '\\',
                '/' => '/',
                'b' => '\u{0008}',
                'f' => '\u{000c}',
                'n' => '\n',
                'r' => '\r',
                't' => '\t',
                _ => return None,
            };
            output.push(decoded);
            escaped = false;
        } else if ch == '\\' {
            escaped = true;
        } else if ch == '"' {
            return Some((output, idx + ch.len_utf8()));
        } else {
            output.push(ch);
        }
    }
    None
}
